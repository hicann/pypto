...
- [ ] **基本信息完整**：Pass 名称、Op 数量、目标耗时计算正确
- [ ] **优化概览准确**：优化前后数据来自实际测量，提升幅度计算正确
- [ ] **每项优化详情完整**：包含问题描述、优化方案、代码变更、性能影响
- [ ] **火焰图路径有效**：所有 SVG 路径指向实际存在的文件
- [ ] **perf 数据真实**：表格中的函数名和占比来自实际 perf report
- [ ] **性能汇总一致**：各优化项贡献之和 ≈ 总提升幅度
- [ ] **经验总结有价值**：可复用模式描述清晰，后续建议具体可行
