...
⛔ **Stage 2 completion:** (a) shape comments on all intermediates (b) Golden function inventory exists in plan (c) original-vs-normalized test passes **(d) `GOLDEN_PERF_REPORT.md` exists with Op Performance section (generated by `pypto-golden-generate/scripts/profile_golden.py` §15)**. **Do not enter Stage 3 until all hold.**
