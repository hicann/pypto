...
**D10 特殊规则**：当 skill 中不存在知识性内容时，D10 自动获得满分。
...
```
total_score = Σ dimension_scores (for D1 through D10)
```
...
注：区间采用数学标准表示法，`[a, b)` 表示包含 a 但不包含 b。

## 问题聚合

来自静态检查、语义检查与知识规则检查的发现会按位置聚合为 **issues**：
