...
**示例**：
- ✅ PASS：`"生成 PyPTO 算子的 golden 参考实现。当需要创建验证基准、写 golden 函数时使用。"` — 同时回答"做什么"(生成golden)和"何时使用"(需要验证基准时)
- ❌ FAIL：`"本技能用于 PyPTO 算子开发流程。"` — 未说明具体产出，触发条件模糊
...
**示例**：
- ✅ PASS：`"...触发词：生成 golden、写验证代码、帮我写 golden"` — 包含用户自然会说出的短语
- ❌ FAIL：`"本技能实现基于规格信息的参考代码生成功能。"` — 仅技术描述，无触发短语
...
### R48 — description 应该 pushy 一些，避免在技能本应发挥作用的场景下却不使用（S3）

**检查**：`description` 应扩展触发范围，避免仅列出显式关键词。应包含 pushy 触发短语，让 AI 在相关场景下主动使用该技能。

**证据标准**：引用 description 内容。检查是否包含扩展触发范围的短语，如 "whenever"、"including"、"or any related to"。若仅有显式关键词列表，则标注为警告。

**判定**：
- `PASS`：description 包含扩展触发短语，主动覆盖更多场景
- `WARN`：description 仅列出显式关键词，可能遗漏应使用的场景
...
**示例**：
- ✅ PASS：`"运行脚本并捕获输出"` — 祈使语气，直接明确
- ❌ FAIL：`"脚本应该被执行，输出应当被检查"` — 被动语态，指令模糊
...
### R46 — 代码块应带有语言标注，纯文本块除外（S3）

**检查**：检查所有代码块，判断内容是否为代码（Python, Bash, JSON, YAML, SQL 等）还是纯文本（URL、简单示例、键值对）。代码块应有语言标识符，纯文本块可以跳过。

**证据标准**：引用缺少语言标注的代码块，并说明其内容类型。若为纯文本块则注明 "PASS"。

**判定**：
- `PASS`：所有代码块都有语言标注，或纯文本块已正确跳过
- `FAIL`：代码块缺少语言标注，且内容明显为代码
...
**示例**：
- ✅ PASS：`"验证语法：运行 \`python3 scripts/validate.py\`"` — 提供具体命令
- ❌ FAIL：`"验证代码是否符合规范"` — 未说明如何验证，缺少执行方式
...
**示例**：
- ✅ PASS：`"阶段 1：读取规则 → 阶段 2：运行脚本 → 阶段 3：生成报告"` — 有编号阶段，顺序清晰
- ❌ FAIL：指令散落在各段落，无明确执行顺序，需读者自行推断
