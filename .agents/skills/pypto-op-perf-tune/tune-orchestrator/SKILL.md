...
|-----------|---------|---------|--------|---------|
| ITER_START | 根据子技能指南选择一个优化点 | 确定了要改什么参数 | ITER_MODIFY | 无优化点可选 → 退出迭代循环（进入对应 PHASE_SUMMARY） |
...
**清单生成规则**：
1. 读取子技能 SKILL.md 中的优化检查清单（如 tune-frontend 的 P0-P4 检查项）
2. 针对当前算子代码逐项分析，标记哪些适用
3. 适用的项列入调优点清单，不适用的标注原因
4. 清单格式：
...
| # | 优化方向 | 具体优化点 | 适用 | 状态 |
|---|---------|-----------|------|------|
| 1 | 任务粒度 | Matmul M轴利用率 | ✅适用 | ⏳待尝试 |
| 2 | TileShape | Cube推荐配置 | ✅适用 | ⏳待尝试 |
| 3 | 数据操作 | reshape inplace | ❌不适用(无循环) | - |
```
