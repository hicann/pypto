...
扫描算子中 op 之间的数据依赖关系，识别基本块拆分模式。多对一/一对多/多对多模式会导致子图无法合并、增加 GM 搬运和调度开销。

**分析方法**：
1. 逐个 op 追踪其输出 tensor 被哪些后续 op 消费（消费者计数）
2. 逐个 op 追踪其输入 tensor 来自哪些前序 op（生产者计数）
3. 按以下分类标注每个 op 的依赖模式：

| 模式 | 定义 | 示意图 | 性能影响 |
|------|------|--------|---------|
| 一对一 | op 的输出恰好被下一个 op 消费 | `A→B` | 最优，子图可合并 |
| 一对多(fan-out) | op 的输出被 ≥2 个 op 消费 | `A→{B,C}` | 每个消费者分支独立子图，重复搬运相同数据 |
| 多对一(fan-in) | op 的输入来自 ≥2 个前置 op | `{A,B}→C` | 分支结果需 GM 落地后 merge 再计算 |
| 多对多 | 同时存在 fan-in 和 fan-out | `{A,B}→{C,D}` | 最复杂，搬运开销叠加 |

**审查表格**：

| # | 模式 | 生产者(op,行) | 共享Tensor | 消费者(op,行) | 是否热循环内? | 优化建议 |
|---|------|--------------|-----------|--------------|-------------|---------|
| 1 | 一对多 | cast(L80) | xe_fp32 | sub(L85), add(L86) | ✅ | Vector合图自动合并相同数据源的消费者 |
| 2 | 多对一 | sub(L85), add(L86) | ye_fp32/yo_fp32 | cast(L87) | ✅ | 调整TileShape使多对一变为两个一对一链 |
| 3 | 一对多 | gathermask(L72) | x_e/x_o/c_e/c_o/s_e/s_o | cast×6(L79-L84) | ✅ | 调整TileShape减少gathermask次数 |

**关键检查项**：
- [ ] 是否存在**一对多 fan-out**导致重复搬运？（优化：Vector 合图将多个消费者合并到同一子图，消除中间 GM 落地）
- [ ] 是否存在**多对一 fan-in**导致 GM 中间结果？（优化：调整 TileShape 使多对一拆解为多个一对一链，避免结果在 GM 上汇合后再计算）
- [ ] 是否存在**多对多**复杂结构？（优化：先拆分基本块为多对一+一对多的组合，再逐级优化；或通过合图一次性合并）
- [ ] fan-in/fan-out 是否在**最内层热循环**中？（优先级最高，调度开销叠加最明显）
- [ ] 共享 tensor 是否可通过调整 TileShape 减小数据量，从而降低 fan-out 的搬运开销？
...
5. **基本块依赖模式分析表**（按 A4 新增模板填写，标注 op 间一对多/多对一/多对多模式）
