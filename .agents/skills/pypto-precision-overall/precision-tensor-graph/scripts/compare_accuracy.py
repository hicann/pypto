...
def compare_with_golden(jit_data, golden_data, name, dtype=None, verbose=True, custom_rtol=None, custom_atol=None):
    ...
    if jit_data.dtype in fp8_dtypes:
        jit_data = jit_data.to(torch.float32)
        golden_data = golden_data.to(torch.float32)

    if jit_data.dtype == torch.bool:
        jit_data = jit_data.to(torch.int8)
        golden_data = golden_data.to(torch.int8)
...
def main():
    ...
    for checkpoint_name in checkpoints:
        ...
        if jit_data is None:
            logger.warning(f"  跳过检查点 {checkpoint_name}: jit 数据读取失败")
            results.append((checkpoint_name, False))
            continue
