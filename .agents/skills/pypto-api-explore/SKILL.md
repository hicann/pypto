...
#### Explore subagent 1: `docs/zh/` — API 文档与约束

**搜索范围**：`docs/zh/` 目录

**任务**：
1. 查 `docs/zh/api/operation/index.md` 确认 API 存在性
2. 读取具体 API 文档 `docs/zh/api/operation/pypto-*.md` 获取参数和约束
3. 未找到 → 标记 unsupported，尝试 substitute 方案
4. 提取入口约束（`docs/zh/api/others/pypto-from_torch.md`）：dtype、contiguous、format
5. 提取 API 约束：dtype 支持、shape 范围、广播规则、特殊值限制
6. 提取 Tiling 约束（`docs/zh/api/config/pypto-set_vec_tile_shapes.md`、`docs/zh/api/config/pypto-set_cube_tile_shapes.md`）
7. 查阅 `docs/zh/api/datatype/` 获取 DataType、TileOpFormat 枚举信息
...
#### Explore subagent 3: `examples/` — 官方示例（API 用法参考）

> **定位**：`examples/` 仅作 **API 用法参考**，不是 production 实现标准。当 example 的写法与 lint / 门禁冲突时，**以 lint 为准**，不得以「example 这样写」为由判定 lint 误报。
...
|----------|------|----------|--------|
| **1: docs/zh/** | `docs/zh/api/operation/index.md` | API 列表，确认存在性 | **入口** |
| | `docs/zh/api/operation/pypto-*.md` | 具体 API 文档 | **主要** |
| | `docs/zh/api/others/pypto-from_torch.md` | 入口约束 | **必查** |
| | `docs/zh/api/config/pypto-set_vec_tile_shapes.md` | Vector Tiling | 条件 |
| | `docs/zh/api/config/pypto-set_cube_tile_shapes.md` | Cube Tiling | 条件 |
| | `docs/zh/api/datatype/` | DataType、TileOpFormat 枚举 | 参考 |
