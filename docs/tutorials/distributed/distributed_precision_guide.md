...
> **注意**：以下计算采用线性误差估计模型，实际浮点累加误差遵循更复杂的规律（Kahan 求和公式），当数据范围差异大时，误差可能显著偏离线性估计。建议在实际验证时，根据数据分布特点调整阈值。
...
> **说明**：`ASCEND_PROCESS_LOG_PATH` 表示日志的落盘路径，设置方法请参考 [环境变量配置说明](/docs/trouble_shooting/README.md)。
...
- 验证切分逻辑是否正确，可参考 [通信算子切块设置指南](/docs/tutorials/distributed/distributed_operation_tiling_guide.md)
...
3. **开启 device 日志并指定落盘路径**：设置方法请参考 [环境变量配置说明](/docs/trouble_shooting/README.md)。
...
3. **查看打印结果**：打印数据位于指定的device日志落盘路径

4. **分析打印结果**：结合 Pass 图分析 TileOp 实际运行时的入参信息（addr、shape、offset 等）是否符合预期，例如数据拷贝的大小和位置是否正确。

### 定位出现问题的OP

定位首个出现精度问题的OP，详细方法参考 [精度调试指南](/docs/tutorials/debug/precision.md) 中的缩小问题规模和工具自检和分析章节。
...
# 注意：torch 在 CPU 上执行 BF16 术运算时，会自动转换为 FP32 做加法，最后将结果转为bf16
