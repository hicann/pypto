...
PowAlgorithm定义了指数操作的精度模式，用于控制指数计算时的精度处理方式。

## 原型定义

```python
class PowAlgorithm(enum.Enum):
...
# 使用高精度模式
out = pypto.pow(a, b, pypto.PowAlgorithm.HIGH_PRECISION)

# 使用指令模式
out = pypto.pow(a, b, pypto.PowAlgorithm.INTRINSIC)
