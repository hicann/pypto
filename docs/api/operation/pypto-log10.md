...
```python
pypto.log10(input, precision_type=pypto.LogAlgorithm.INTRINSIC) -> Tensor
```
...
y = pypto.log10(x, pypto.LogAlgorithm.HIGH_PRECISION)
```
...
```python
x = pypto.tensor([3], pypto.DT_FP16)
y = pypto.log10(x, pypto.LogAlgorithm.INTRINSIC)
```
