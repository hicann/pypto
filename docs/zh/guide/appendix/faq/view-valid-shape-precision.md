...
当`view`接口的输入Tensor没有一个正确的valid_shape时，框架无法正确推导出输出的valid_shape。
...
当输入的valid_shape依赖别的Tensor标识，必须传入`valid_shape`。如下所示场景，`q0`的valid_shape `curSeq`来自于另外一个Tensor，无法通过推导得到：
