...
- Kernel业务输入输出参数名称与算子原型一致，形参排列顺序与Host侧Kernel参数下发顺序一致，因为离线交付按位置绑定Host侧下发参数与Device侧Kernel形参。参数可使用[`Tensor`](../../../../../api/pro_api/SIMD-API/basic_data_structures/Tensor.md)或[`Ptr`](../../../../../api/pro_api/SIMD-API/basic_data_structures/Ptr.md)声明。
