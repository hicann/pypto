...
配套阅读：[TilingData](tiling_result_transfer.md#tilingdata)、[尾块处理](#尾块处理)、
[Tensor创建和操作](../tensor_creation_and_operations.md)。
...
TilingData的字段类型、数组和布局规则参见[TilingData](tiling_result_transfer.md#tilingdata)。三种运行时参数
...
传递运行时shape。完整用法参见[TilingKey](tiling_result_transfer.md#tilingkey)。
...
- **处理非整除场景**：Tile尺寸无需整除Tensor shape，可通过向上取整计算Tile数量，
  并使用`set_validshape`处理尾块（参见[尾块处理](#尾块处理)）。
...
同步（细节见[Tensor创建和操作](../tensor_creation_and_operations.md)）：
...
- 这里`M_SIZE`恰好被`TILE_M`整除；不整除时把`num_cores`改成
  `min(info.vector_core_num, ceildiv(M, TILE_M) * ceildiv(N, TILE_N))`并按[尾块处理](#尾块处理)
  处理边界。
...
完整、可执行的`K=128`示例参见[Matmul算子（SIMD）快速入门](../../../../quick_start/pro/matmul_simd.md)。`K`大于单个K Tile时，需要沿K轴循环并使用累加语义。

多核、TilingData与尾块处理的组合用法参见[TilingData](tiling_result_transfer.md#tilingdata)和
[尾块处理](#尾块处理)。
