...
1. **检查张量维度**：确认张量的维度为 2 - 4 维，符合要求。

#### 0xA0003 INVALID_TENSOR_SHAPE

1. **检查张量形状**：确保张量形状为 2 - 4 维，且每个维度的大小为正整数，避免出现零维、负维等非法情况。
...
1. **检查创建的shmem_tensor大小**：确认每个创建的 shmem_tensor 的总字节大小（shape各维度乘积 × dtype字节大小）小于 200MB（1024*1024*200字节）。
...
1. **检查通信域组数量**：确认创建的通信域组数量不超过最大限制（当前最大支持 2 个通信域组）。
