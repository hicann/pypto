...
| `relu_pre_mode` | 输入 | 默认 `None`（不融合 ReLU）；可取 `pl.ReluPreMode.NormalRelu`；与 `scale` 为 `Tile`（per-channel）时互斥 |
...
| `phase` | 输入 | `pl.STPhase.Unspecified`/`pl.STPhase.Partial`（中间累加步）/`pl.STPhase.Final`（最终步）；用硬件 unit_flag 接管 cube/fixp 间的握手，仅在 matmul 多步累加写回 GM 时使用；与 `scale` 为 `Tile`（per-channel）时互斥 |
