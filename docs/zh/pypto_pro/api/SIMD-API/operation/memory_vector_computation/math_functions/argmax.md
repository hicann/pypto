...
沿指定维度查找 `src` tile 的最大元素索引并写入 `out`。`dim=0` 沿最后一维查找每行最大元素的列索引；`dim=1` 沿第一维查找每列最大元素的行索引。
...
下面是一个完整 kernel：对 64×128 FP16 源 tile 做行向取最大值索引，输出 `[64, 1]` INT32。vector kernel 开 `auto_mutex`，同步由 `make_tile_group` 自动管理。
