...
先对两个 tile 做逐元素减法，再对结果施加 ReLU 激活，最后做数据类型转换。

## 函数原型

```python
pypto_pro.language.sub_relu_cast(out, lhs, rhs, *, target_type, mode=pl.RoundMode.CAST_ROUND)
```
...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| `out` | 输出 | 目标 tile，存放先减后 ReLU 再类型转换的结果 |
...
下面是一个完整 kernel：从 GM 载入两个 FP16 输入，用 `pypto_pro.language.sub_relu_cast` 先减后 ReLU 再转 FP32 写回 GM。vector kernel 开 `auto_mutex`，同步由 `make_tile_group` 自动管理。
