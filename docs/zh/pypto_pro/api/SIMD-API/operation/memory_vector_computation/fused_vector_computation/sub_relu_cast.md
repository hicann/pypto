...
先对两个tile做逐元素减法，再对结果施加ReLU激活，最后做数据类型转换。
...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| `out` | 输出 | 目标tile，存放先减后ReLU再类型转换的结果 |
| `lhs` | 输入 | 左操作数tile（被减数） |
| `rhs` | 输入 | 右操作数tile（减数） |
...
下面是一个完整kernel：从GM载入两个FP16输入，用`pypto_pro.language.sub_relu_cast`先减后ReLU再转FP32写回GM。vector kernel开`auto_mutex`，同步由`make_tile_group`自动管理。
