...
根据mask，对源操作数`src0`、`src1`进行按元素求差操作，将结果写入目的操作数`dst`，计算公式如下：
...
若需要获取减法借位输出，请参考[vf.subc](subc.md#功能说明)。

## 函数原型

```python
sub(src0, src1, preg, mode: Optional[MergeMode] = None) -> dst
```
...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| `src0` | 输入 | 源操作数0，[reg_tensor](../reg_tensor.md)，源操作数`src0`与目的操作数`dst`的数据类型保持一致。支持的数据类型为：DT_INT8、DT_UINT8、DT_INT16、DT_UINT16、DT_INT32、DT_UINT32、DT_FP16、DT_BF16、DT_FP32。 |
