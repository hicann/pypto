...
3. Device端的AI Core通过`load`/`load_tile`将数据从Global Memory搬入片上Buffer，完成Tile级别的计算，再通过`store`/`store_tile`将结果写回Global Memory。
4. Host端同步后访问结果。
...
- **多核集群层面采用SPMD编程模型**：每个AI Core抽象为一个Block，每个Block执行同一份Kernel代码，并根据Block索引处理不同的数据分片。
- **单核计算层面采用SIMD并行机制**：单个AI Core内部通过一条指令同时处理多个同构数据元素，完成向量、矩阵或融合计算。

在PyPTO Pro中，一个**Block对应一个AI Core**。启动Kernel时，同一份Kernel代码会在`block_dim`个AI Core上执行。每个AI Core通过以下接口获取自身编号和启动核数：

- `pl.get_block_num()`获取本次Kernel启动的Block总数。
- `pl.get_block_idx()`获取当前Block的编号。
- `pl.get_subblock_idx()`获取AI Core内部的Vector子核编号。

```text
block_dim 个 block（AI Core）        ← launch时决定，get_block_num()读取
   └── 每个block内2个subblock        ← Vector双子核，get_subblock_idx()读取
```
...
AI Core片上存储空间有限，无法一次性加载超大尺寸Tensor。实际开发中通常迭代完成分块搬运、分批计算和结果写回，并通过TileGroup的N-Buffer流水减少数据搬运带来的等待。
...
2. 不同算子的Buffer复用方式差异较大。
3. offset计算和底层指令参数填写复杂且容易出错。
4. 核内、核间流水排布和同步插入较为复杂。

PyPTO Pro保留由开发者控制多核Tiling切分和Buffer复用的能力，并通过以下机制简化编程：

### Tile抽象

PyPTO Pro使用二维Tile描述核内Buffer。开发者只需表达Tile在Global Memory中Tensor上的坐标，无需手工将多维坐标转换为一维offset；Tile API同时封装了底层指令参数。

### TileGroup与自动核内同步

对于Cube、Vector核内流水，PyPTO Pro通过`pl.make_tile_group`将同一流水线中轮转使用的多块Tile封装为一组。开发者通过`next()`和`current()`获取当前Tile，并通过`mutex_ids`标识Buffer。使用`@pl.jit(auto_mutex=True)`编译Kernel时，框架会根据Tile的mutex信息自动插入核内同步。
...
| API层级 | 编程模式 | 特点 | 主要用途 |
|----------|----------|------|----------|
| **Tile API** | 基于Tile编程 | 通过`make_tile`/`make_tile_group`分配片上Buffer，使用`auto_mutex=True`自动管理核内同步与N-Buffer流水 | 适配大多数算子开发场景，兼顾硬件控制能力和开发效率 |
...
2. **定义Tile与TileGroup**：通过`pl.TileType`描述Tile的Shape、数据类型和内存空间，通过`pl.make_tile`或`pl.make_tile_group`分配片上Buffer。
...
PyPTO Pro以SPMD+SIMD编程范式将同一份Kernel分发到多个AI Core执行，并通过Tile和Reg两级接口表达Vector、Cube及融合计算。开发者显式控制多核数据切分和片上Buffer使用，框架则通过Tile、TileGroup和stage机制简化指令参数、核内同步及核间流水的表达。
