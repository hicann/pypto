...
- `python/tests/ut/block/frontend/a5/element_wise/test_add.py`（最简单的动态shape多核加法）。
...
完整示例见`python/tests/ut/block/frontend/a5/fa/test_fa_tilingkey_attn_mask.py`。
...
方法论 —— 直接对应Ascend C的Tiling设计指南
（`.../ops/ascendc-tiling-design`）。指南把所有算子都要考虑的通用要素归为四类，下面给出
它们在PyPTO Pro里的落地方式。

### 多核切分：负载均衡

指南（`SKILL.md` §1）的要点：**每个核的任务量尽量相等；相邻数据尽量同核；tile粒度适中
（太小调度开销大，太大并行度低）。**

以Elementwise为例（`references/elewise/tiling.md`），host侧算核数的思路是"保证每个核
至少处理一个最小阈值（如4KB），再对齐到硬件友好的粒度"：
...
指南（§2 / elewise §2）的要点：**受UB容量限制（Ascend 950PR/Ascend 950DT为248KB），
