...
- `python/tests/ut/block/frontend/a5/element_wise/test_tiling_op.py` —— 一个简单、
  可执行的端到端示例（本文档贯穿使用）。
...
## . 完整、可执行示例 —— 通过数组字段的运行时算子选择器

这是`python/tests/ut/block/frontend/a5/element_wise/test_eltwise_dynamic_rank.py`
tiling结构体混合了数组字段与标量字段；真正的算子选择器位于`opkind[4]`，kernel据此分支以在运行时选择 加 / 减 / 乘。
...
TILE_M = 128
TILE_N = 128
TILE_ELEMS = TILE_M * TILE_N  # 16384 elements per tile

#定义TilingData，包含shape的维度和运算的类型
@dataclass
class AddTiling:
    shape: int[4]       # per-dim sizes; unused trailing dims are 1
...
def add_dynrank_kernel(
    ...
    # Collapse the (rank 2..4) logical shape to a flat element count. Unused
    # trailing dims are 1 on the host, so this product is correct for any rank.
    numel = tiling.shape[0] * tiling.shape[1] * tiling.shape[2] * tiling.shape[3]

    # Re-view each flat buffer as 2D [rows, TILE_N]: one column of tiles, iterate
    # the row-tiles. (numel is a multiple of TILE_M*TILE_N, see _run_case.)
    rows = numel // TILE_N
    tensor_x = pl.make_tensor(x, [rows, TILE_N], [TILE_N, 1])
    tensor_y = pl.make_tensor(y, [rows, TILE_N], [TILE_N, 1])
    tensor_z = pl.make_tensor(z, [rows, TILE_N], [TILE_N, 1])
    ...
    with pl.section_vector():
        ...
        row_tiles = rows // TILE_M       # total tiles over the whole flattened buffer

        # Each core strides over the flat tile space.
        for t in pl.range(core_id, row_tiles, num_cores):
            tile_a = a_db.next()
            tile_b = b_db.next()
            tile_c = c_db.next()
            pl.load_tile(tile_a, tensor_x, [t, 0])
            pl.load_tile(tile_b, tensor_y, [t, 0])
            ...
            pl.store_tile(tensor_z, tile_c, [t, 0])
...
def _run_case(shape, opkind, ref_fn, op_name):
    ...
    for s in shape:
        numel *= s
    assert numel % TILE_ELEMS == 0, f"numel {numel} must be a multiple of {TILE_ELEMS}"

    x = torch.rand(shape, device=device, dtype=dtype)
    y = torch.rand(shape, device=device, dtype=dtype)
    z = torch.empty(shape, device=device, dtype=dtype)

    # Pad the shape to length 4 with 1s so the in-kernel product is rank-agnostic.
    dims = [1] * MAX_RANK
    for i in range(rank):
        dims[i] = shape[i]
    ...
    row_tiles = numel // TILE_ELEMS
    num_cores = min(32, row_tiles)
...
def test_add_dynamic_rank():
    shapes = [
        [512, 512],          # rank 2 -> numel 262144 = 16 tiles
        [8, 256, 256],       # rank 3 -> numel 524288 = 32 tiles
        [2, 4, 256, 256],    # rank 4 -> numel 524288 = 32 tiles
    ]
    for shape in shapes:
...
- 混合`int[N]`与标量字段的tiling类；
- kernel体内的**数组元素访问**（`tiling.opkind`）用以驱动不同的计算分支
