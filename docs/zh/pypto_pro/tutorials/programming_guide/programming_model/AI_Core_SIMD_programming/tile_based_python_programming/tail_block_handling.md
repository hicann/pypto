...
- `python/tests/ut/block/frontend/a5/element_wise/test_eltwise_dynamic_rank.py`
  （四类尾块的完整覆盖，本文贯穿使用）。
- `python/tests/ut/block/frontend/a5/docs/test_doc_dcci_atomic_validshape.py`
  （最小`set_validshape`示例）。
- `python/tests/ut/block/frontend/a5/datacopy/test_fillpad.py`（`pad` + `fillpad`）。
...
```python
def fillpad(out, src) -> None            # 把 src 的 padding 区按 out 的 pad 模式填好
def fillpad_inplace(out, src) -> None    # dst 与 src 同址的原地版本
def fillpad_expand(out, src) -> None     # 扩展（列展开）版本
```

示例（照搬`test_fillpad.py`）：`src`声明动态有效形状，`dst`声明`pad=zero`；运行时把
...
| API                        | dst / src关系      | 用途                                         |
|----------------------------|---------------------|----------------------------------------------|
| `pl.fillpad(dst, src)`     | 不同址              | 把`src`拷到`dst`并按`dst.pad`填充边界。|
| `pl.fillpad_inplace(dst, src)` | **同址**（`addr`相同） | 原地填充，省一块buffer。               |
| `pl.fillpad_expand(dst, src)`  | dst列数 > src   | 列方向**展开**并填充（如`[8,8]` → `[8,16]`）。|

`fillpad_expand`例子（`test_fillpad.py`）：`src`有效区`[5, 7]`，`dst`是`[8, 16]`且
...
with pl.section_vector():
    ...
    pl.fillpad_expand(dst, src)      # [8,8] 有效 5x7 -> [8,16] 其余清零
    pl.store(z, dst, [0, 0])
```
...
with pl.section_vector():
    ...
    pl.fillpad_inplace(src, src)         # 无效行填 min（pad=min）
...
`TileType.compact`（`pl.CompactMode`）控制tile在片上buffer里的紧凑摆放方式：

| `CompactMode`  | 含义                                          |
|----------------|-----------------------------------------------|
| `null`         | 不紧凑（默认），按物理shape常规摆放。      |
| `normal`       | 常规紧凑模式。                                |
| `row_plus_one` | "行数 +1" 紧凑模式（某些算子的对齐需求）。   |

整数与枚举对应（`make_tile`也接受整数）：`compact=1` == `CompactMode.normal`，
`compact=2` == `CompactMode.row_plus_one`，`compact=0`/不给 == `null`。codegen把它们分别
翻成`CompactMode::Normal` / `CompactMode::RowPlusOne` / `CompactMode::None`
（`type_converter.cpp:ConvertCompactModeToPTOValue`）。
...
下面是`python/tests/ut/block/frontend/a5/element_wise/test_eltwise_dynamic_rank.py`的
