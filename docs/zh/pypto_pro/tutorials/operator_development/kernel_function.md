...
Tensor输入输出通常使用[`pypto_pro.language.Tensor`](../../api/SIMD-API/basic_data_structures/Tensor.md)标注，并指定张量的形状和数据类型：
...
| [`pypto_pro.language.Ptr[dtype]`](../../api/SIMD-API/basic_data_structures/Ptr.md) | 裸指针输入输出；常与TilingData配合重建动态shape的Tensor视图 |
...
核函数内部使用[`pypto_pro.language.TileType`](../../api/SIMD-API/basic_data_structures/TileType.md)定义Tile类型，并通过[`pypto_pro.language.make_tile_group`](../../api/SIMD-API/operation/resource_management/make_tile_group.md)等接口分配片上内存：
...
计算逻辑需要放在[`pypto_pro.language.section_vector()`](../../api/SIMD-API/operation/controlflow/section_vector.md)上下文中，开启`auto_mutex=True`后，搬运与计算间的流水同步由框架按Tile的mutex自动插入：
