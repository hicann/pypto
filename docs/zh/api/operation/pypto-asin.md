...
计算输入Tensor中每个元素的反正弦值，逐元素运算。结果落在 $[-\pi/2, \pi/2]$ 区间内。当输入元素的绝对值大于 1 时，对应输出为 NaN。
...
返回Tensor类型。其Shape与输入Tensor一致，数据类型与输入Tensor一致，其元素为输入Tensor对应元素的反正弦值。
