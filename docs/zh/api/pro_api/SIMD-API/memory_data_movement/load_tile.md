...
| tile_offsets | 输入 | 搬运起始位置，List[int或Scalar]类型。列表中的第i项对应src_tensor的第i维。<br>- order选中的维度：按Tile块编号寻址，对应编号乘以dst_tile的shape中对应维度的大小后得到绝对元素坐标。<br>- order未选中的维度：按绝对元素坐标寻址，用于固定高维Tensor的其他维度。 |
