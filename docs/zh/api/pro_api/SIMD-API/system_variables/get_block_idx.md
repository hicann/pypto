...
在Vector段中，该接口返回展平后的全局AIV逻辑索引，可直接用于数据分片和偏移计算。混合Kernel在Vector段做跨步切分时，工作单元总数为get_block_num() × get_subblock_num()。

## 调用示例

下面是一个仅包含Vector段的多核Kernel：通过Kernel[None, NUM_CORES]形式启动2个逻辑Block，每个AIV用pypto_pro.language.get_block_idx()获取全局逻辑索引，并处理64行逐元素加法。
