...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| pattern | 输入 | 可选，掩码模式，pattern参数决定mask_reg中哪些元素被设置为有效（1），哪些被设置为无效（0），对应[MaskPattern](../types/MaskPattern.md)类型。支持的模式见[约束说明](#约束说明)，默认pypto_pro.language.MaskPattern.ALL。 |
