...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| src0 | 输入 | 源操作数，[reg_tensor](../reg_tensor.md)。支持的数据类型为：DT_INT8、DT_UINT8、DT_INT16、DT_UINT16、DT_FP16、DT_BF16、DT_INT32、DT_UINT32、DT_FP32、DT_INT64、DT_UINT64。 |
...
def example_vf_int64(src_a, src_b, dst_tile):
    ...
    reg_a = vf.load_align(src_a, 0)
    reg_b = vf.load_align(src_b, 0)
    eq_mask = vf.eq(reg_a, reg_b, cmp_mask)
    reg_out = vf.select(reg_a, reg_b, eq_mask)
    vf.store_align(dst_tile, reg_out, preg)
