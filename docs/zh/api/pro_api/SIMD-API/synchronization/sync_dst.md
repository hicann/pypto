...
用于同一AI Core内不同流水之间的同步。当wait_pipe指定的流水执行到本接口时，如果对应标志位为0，wait_pipe中位于本接口之后的操作将被阻塞；如果对应标志位为1，则将标志位清零，并继续执行后续操作。该接口需要与[pypto_pro.language.system.sync_src](sync_src.md)配对使用。
