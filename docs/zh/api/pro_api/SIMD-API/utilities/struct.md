...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| type_name | 输入 | 结构体类型名，必须是字符串常量，不能是变量。名称只能包含字母、数字和下划线，不能以数字开头，且不能是C++关键字。 |
| **fields | 输入 | 字段名和初始值（关键字参数），至少包含一个字段，字段名不可重复。字段名只能包含字母、数字和下划线、不能以数字开头，且不能是C++关键字。标量字段初始值支持整数、浮点数、布尔值或Scalar表达式；列表用于创建定长数组字段，例如offsets=[0, 0, 0, 0]。数组长度和元素类型必须在编译期确定，数组字段至少包含一个元素，元素应为同一数据类型。不支持**kwargs展开，也不支持将Tensor、Tile、Ptr，或嵌套的具名结构体（make_tuple、struct的结果）作为字段。字段值可通过message.field = value修改，数组字段可通过message.field[index]读写。可在for循环内创建struct；if/else分支内只能修改已存在字段，不能只在某个分支创建新struct。 |

## 约束说明

### 类型名和字段布局

- 同一个Kernel中，同一type_name只能对应一种字段定义。重复使用相同类型名时，字段名、顺序、标量类型和数组长度必须完全一致。
