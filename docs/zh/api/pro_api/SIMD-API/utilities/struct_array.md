...
| type_name | 输入 | 结构体类型名，必须是字符串常量，作为第二个位置参数。名称只能包含字母、数字和下划线，不能以数字开头，且不能是C++关键字。 |
| **fields | 输入 | 字段名和初始值（关键字参数），至少一个关键字参数。字段名须为合法标识符且不可重复。字段名只能包含字母、数字和下划线、不能以数字开头，且不能是C++关键字。初始值可为整型、浮点型、标量表达式或列表（数组字段，如arr=[0, 0, 0, 0]），数组字段至少包含一个元素，且所有元素必须是同一数据类型，不允许混型。不支持**kwargs展开，也不支持将具名嵌套结构体（make_tuple、struct的结果）作为字段。 |
