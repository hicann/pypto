...
以下示例先通过`vf.pack`将INT64压缩为UINT32（pack的目的数据类型须为无符号），再通过`vf.bit_cast`按位重解释为INT32，最后通过`vf.unpack`恢复为INT64，验证往返一致性。`vf.unpack`对INT32源执行符号扩展（高位填充符号位），正确还原原始INT64值。
