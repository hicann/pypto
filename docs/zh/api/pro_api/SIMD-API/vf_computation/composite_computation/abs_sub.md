...
该接口根据preg将src0与src1逐元素相减再求绝对值，计算结果写入dst。
...
| 参数 | 输入/输出 | 说明 |
|---|---|---|
| src0 | 输入 | 源操作数0，[reg_tensor](../reg_tensor.md)，支持的数据类型为：DT_FP16、DT_FP32、DT_INT64。 |
...
返回dst目标[reg_tensor](../reg_tensor.md)，支持的数据类型和src0中的说明一致。
...
def test_example_int64():
    ...
    torch.npu.synchronize()
    torch.testing.assert_close(out, torch.abs(a - b), rtol=0, atol=0)
