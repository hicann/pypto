...
将lhs与rhs逐元素比较是否不相等，生成bit-packed掩码。rhs可以是Tile或标量。掩码输出为DT_UINT8类型，通常配合[pypto_pro.language.select](../selection/select.md)使用。

## 函数原型

```python
pypto_pro.language.ne(out: Tile, lhs: Tile, rhs: Union[Tile, Scalar]) -> None
```
...
```python
pl.ne(mask, lhs, rhs)  # Tile-Tile比较：lhs != rhs
pl.ne(mask, lhs, 0)    # Tile-Scalar比较：lhs != 0
```
