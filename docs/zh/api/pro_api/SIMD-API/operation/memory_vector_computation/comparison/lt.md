...
将lhs与rhs逐元素比较，当lhs小于rhs时生成真值，结果以bit-packed掩码输出。rhs可以是Tile或标量。掩码输出为DT_UINT8类型，通常配合[pypto_pro.language.select](../selection/select.md)使用。

## 函数原型

```python
pypto_pro.language.lt(out: Tile, lhs: Tile, rhs: Union[Tile, Scalar]) -> None
```
...
```python
pl.lt(mask, lhs, rhs)  # Tile-Tile比较：lhs < rhs
pl.lt(mask, lhs, 0)    # Tile-Scalar比较：lhs < 0
```
