...
返回两个源操作数中的较大值，数据类型与输入一致。对于浮点输入，一个操作数为NaN时返回另一个操作数，两个操作数均为NaN时返回NaN。
...
def clamp_lower(
    ...
    output[0, tid] = pl.simt.max(source[0, tid], lower)
...
def simt_max_kernel(
    ...
    with pl.section_vector():
        pl.simt.launch(clamp_lower, threads=256, args=(source, output, lower))
```
