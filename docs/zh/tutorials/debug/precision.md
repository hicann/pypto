...
最新master 分支代码及0.1.1 之后版本（不含0.1.1 版本）支持在运行时在线编译精度工具所需C++ 二进制，不需重新编译安装PyPTO, 但需确认在线编译所需的构建工具符合以下要求：
...
2. 设置golden 数据（可选）
    ...
    如果需要进行tensor_graph验证，需要设置golden 数据：
    ...
    **参数说明**：
    ...
    | `goldens` | List[Union(pypto.Tensor, torch.Tensor)] | 将用户已有的计算基准数据（golden）输出设置到工具中做对比检测。该列表与算子输入、输出参数列表的长度一致、位置对应。若相应位置设置为None，表示跳过该位置的数据对比。**注意：torch.Tensor 的device 属性需为CPU，不支持NPU** |
...
5. 执行结束后，在$\{work\_path\}/output/output\_\*/目录（\*代表时间戳）下生成verify\_\*目录，存放检测结果文件与日志。
    ...
    ├── interpreter.log # interpreter 模块拆分日志（默认记录ERROR / EVENT）
    ...
    - `interpreter.log`：偏向interpreter 执行过程中的拆分日志（当前默认仅ERROR/EVENT 落盘）。
...
7. 使用 `pass_verify_print` 和 `pass_verify_save` 分析中间结果（可选）。
    ...
    **重要说明**：
    ...
    - 这些结果是在主机CPU 上通过模拟执行计算图得到的
    - **与实际在NPU 上板执行的结果可能存在差异，主要用于算法逻辑验证**
...
- 解析dump 的二进制数据（.tdump 文件），提取tensor 数据为 .data 文件
- 自动合并分片tensor 为完整的raw tensor（针对多个task 处理同一raw tensor 的场景）
- 支持codegen pass tensor 对比验证（需配合 `enable_pass_verify` 使用）
...
# 带验证的用法（需先开启enable_pass_verify 并运行算子）
python3 tools/verifier/parse_dump_tensors.py \
    --dump_tensor_path output/output_20260101120000/dump_tensor_20260101120000/device_0 \
...
| 参数 | 必需/可选 | 说明 | 默认值 |
|------|-----------|------|--------|
| `--dump_tensor_path` | 必需 | dump 数据目录路径，指向 `device_x` 目录 | 无 |
| `--verify_path` | 可选 | verify 结果目录路径（包含verify_graph_data_metainfo.csv）| `""`（不对比验证） |
...
|------|------|
| B>taskId | 任务ID |
| ROOT_CALL:opmagic | 算子调用magic 标识 |
| ROOT_CALL:rawmagic | 原始tensor magic 标识 |
| B>validshape | tensor 实际shape |
| B>offset | tensor 在raw tensor 中的偏移 |
| B>rawShape | 原始完整tensor 的shape |
| B>tensorAddr | tensor 内存地址 |
| B>datatype | 数据类型（字符串，如FP32、INT8） |
| IO_FLAG | 输入/输出标记（input/output） |
| B>seqNo | 序列号 |
| B>TIMESTAMP | 时间戳 |
| B>funcId | Function ID |
| ROOT_FUNC:hash | Root Function hash 值 |
| FUNC:hash | Function hash 值 |
...
| result_reason | 未对比原因（NO_CMP 时，如 "unsupported dtype: BOTTOM"） |
...
3. **Shape 处理**：自动处理shape 不一致的对比（取公共部分）
4. **不支持类型**：HF4、HF8、BOTTOM 等类型标记为NO_CMP

**Raw Tensor 合并说明：**

当多个task 处理同一个raw tensor 的不同分片时，脚本会自动：
...
数据输出路径就是acl.json 里面配置的dump_path，在该路径下会生成如下文件：
