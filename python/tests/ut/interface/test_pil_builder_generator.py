...
def test_pil_builder_list_comp():
    with TestParser():
        ...
        def listcomp_two_ifs():
            _var_l = [
                Expr.str(var_x) for var_x in [Expr.int(0), Expr.int(1), Expr.int(2)] if Expr.true(var_x) if var_x != 2
            ]
        ...
        def listcomp_two_fors():
            _var_l = [Expr.str(var_x) for var_x in [Expr.int(0), Expr.int(1)] for var_y in [Expr.int(2), Expr.int(3)]]
...
def test_pil_builder_set_comp():
    with TestParser():
        ...
        def setcomp_two_fors():
            _var_s = {Expr.str(var_x) for var_x in [Expr.int(0), Expr.int(1)] for var_y in [Expr.int(2), Expr.int(3)]}
...
def test_pil_builder_generator_exp():
    with TestParser():
        ...
        @TestParser.test
        def genexp_two_fors():
            g = (Expr.str(var_x) for var_x in [Expr.int(0), Expr.int(1)] for var_y in [Expr.int(2), Expr.int(3)])
            _var_l = list(g)
