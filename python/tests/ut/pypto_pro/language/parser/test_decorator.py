...
class TestExternalFunctionCalls:
    ...
    @staticmethod
    def test_naming_conflict_with_internal_raises_error():
        """External with same name as internal @pl.function raises ParserSyntaxError."""

        @pl.function
        def conflicting(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result
        ...
        with pytest.raises(ParserSyntaxError, match="conflicts with program function"):
            ...
            class Conflict:
                @pl.function
                def conflicting(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                    result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
                    return result

                @pl.function
                def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                    result: pl.Tensor[[64], pl.DT_FP32] = conflicting(x)
                    return result

    @staticmethod
    def test_two_externals_same_name_raises_error():
        """Two different external functions with same .name raises ParserSyntaxError."""

        @pl.function
        def helper(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result
        ...
        @pl.function
        def helper(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:  # noqa: F811
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
            return result
        ...
        with pytest.raises(ParserSyntaxError, match="Conflicting external functions"):
            ...
            class ConflictExt:
                @pl.function
                def func_a(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                    result: pl.Tensor[[64], pl.DT_FP32] = helper_v1(x)
                    return result

                @pl.function
                def func_b(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                    result: pl.Tensor[[64], pl.DT_FP32] = helper_v2(x)
                    return result

    @staticmethod
    def test_external_print_style():
        """Print program with external function calls in unified IR style."""

        @pl.function
        def ext_add_one(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result
        ...
        class Original:
            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = ext_add_one(x)
                return result

        printed = ir.python_print(Original)
        assert "# ir.program: Original" in printed
        assert "ir.call @ext_add_one(x)" in printed
        assert "@ir.program" not in printed

    @staticmethod
    def test_non_function_bare_call_still_errors():
        ...
        with pytest.raises((UnsupportedFeatureError, Exception), match="Unsupported"):
            ...
            class BadCall:
                @pl.function
                def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                    result: pl.Tensor[[64], pl.DT_FP32] = not_a_function(x)  # type: ignore[operator]
                    return result
    ...
    def test_basic_external_call(self):
        ...
        @pl.function
        def double(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
            return result
        ...
        class MyModel:
            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = double(x)
                return result

        @pl.program
        class Expected:
            @pl.function
            def double(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
                return result

            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = self.double(x)
                return result
    ...
    def test_external_return_type_propagation(self):
        ...
        @pl.function
        def ext_square(x: pl.Tensor[[32], pl.DT_INT32]) -> pl.Tensor[[32], pl.DT_INT32]:
            result: pl.Tensor[[32], pl.DT_INT32] = pl.tensor.mul(x, x)
            return result

        @pl.program
        class TypeProp:
            @pl.function
            def main(self, x: pl.Tensor[[32], pl.DT_INT32]) -> pl.Tensor[[32], pl.DT_INT32]:
                y: pl.Tensor[[32], pl.DT_INT32] = ext_square(x)
                return y

        @pl.program
        class Expected:
            @pl.function
            def ext_square(self, x: pl.Tensor[[32], pl.DT_INT32]) -> pl.Tensor[[32], pl.DT_INT32]:
                result: pl.Tensor[[32], pl.DT_INT32] = pl.tensor.mul(x, x)
                return result

            @pl.function
            def main(self, x: pl.Tensor[[32], pl.DT_INT32]) -> pl.Tensor[[32], pl.DT_INT32]:
                y: pl.Tensor[[32], pl.DT_INT32] = self.ext_square(x)
                return y
    ...
    def test_multiple_external_functions(self):
        ...
        @pl.function
        def ext_add(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result

        @pl.function
        def ext_mul(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
            return result
        ...
        class MultiExt:
            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                y: pl.Tensor[[64], pl.DT_FP32] = ext_add(x)
                z: pl.Tensor[[64], pl.DT_FP32] = ext_mul(y)
                return z

        @pl.program
        class Expected:
            @pl.function
            def ext_add(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
                return result

            @pl.function
            def ext_mul(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.mul(x, 2.0)
                return result

            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                y: pl.Tensor[[64], pl.DT_FP32] = self.ext_add(x)
                z: pl.Tensor[[64], pl.DT_FP32] = self.ext_mul(y)
                return z
    ...
    def test_same_external_from_multiple_methods(self):
        ...
        @pl.function
        def shared_helper(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result
        ...
        class SharedExt:
            @pl.function
            def func_a(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = shared_helper(x)
                return result
        ...
        class Expected:
            @pl.function
            def shared_helper(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
                return result

            @pl.function
            def func_a(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = self.shared_helper(x)
                return result
    ...
    def test_aliased_import_uses_original_name(self):
        """Aliased reference uses the function's original .name for the Op."""

        @pl.function
        def original_name(x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
            result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
            return result
        ...
        class AliasTest:
            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = aliased(x)
                return result

        @pl.program
        class Expected:
            @pl.function
            def original_name(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = pl.tensor.add(x, 1.0)
                return result

            @pl.function
            def main(self, x: pl.Tensor[[64], pl.DT_FP32]) -> pl.Tensor[[64], pl.DT_FP32]:
                result: pl.Tensor[[64], pl.DT_FP32] = self.original_name(x)
                return result
