...
"""Frontend runtime example for fixpipe scalar dequant with a DYNAMIC (runtime) scale on A5 CCE.

Unlike the other test_deq_scalar_* tests (which pass pre_quant_scalar as a compile-time int
constant), here the scale is a runtime kernel argument `scale_bits: pl.DT_INT32`,
provided at launch time. This validates the operand refactor: pre_quant_scalar is now a
positional operand (not a compile-time attr), so the scale can be decided at runtime.

The scalar carries the low 32 bits = the float32 bit-pattern of the scale (same encoding as the
constant tests). The CCE codegen adds the signed-int8 bit46 flag from the output dtype at runtime,
so negatives still requantize with the sign preserved.
"""
...
@pl.jit()
def deq_scalar_dynamic_cce_kernel(
    ...
    with pl.section_cube():
        ...
        # DYNAMIC scale: pre_quant_scalar is the runtime kernel arg, not a compile-time constant.
        pl.store(deq_out, acc, [0, 0], pre_quant_scalar=scale_bits)
        pl.system.bar_all()



@pytest.mark.soc("950")
def test_deq_scalar_dynamic_cce():
    device = ST_DEVICE
    ...
    deq_scalar_dynamic_cce_kernel(q, k, deq_out, scale_bits)
    ...
    logging.info("***********cce expected deq int8 output***********")
    logging.info("%s", expected_deq[:8, :8])
    logging.info("***********cce dynamic-scale deq int8 output (top-left 8x8)***********")
    logging.info("%s", deq_out[:8, :8])

    torch.testing.assert_close(deq_out.to(torch.int32), expected_deq.to(torch.int32), rtol=0, atol=0)
    logging.info("cce dynamic-scale deq_scalar output matches expectation.")
