...
"""Frontend runtime example for pl.store(..., fp_tile=...) on A5 CCE."""
...
def store_fp_cce_kernel(
    ...
    fp_out: pl.Tensor[[64, 64], pl.DT_INT8],
):
    with pl.section_cube():
        ...
        pl.store(fp_out, acc, [0, 0], fp_tile=fp_tile)
        pl.system.bar_all()
...
def test_store_fp_cce():
    ...
    store_fp_cce_kernel(q, k, fp_params, raw_out, fp_out)
    ...
    logging.info("***********cce expected store_fp int8 output***********")
    logging.info("%s", expected_fp[:8, :8])
    logging.info("***********cce store_fp int8 output (top-left 8x8)***********")
    logging.info("%s", fp_out[:8, :8])
    logging.info("***********cce store_fp summary***********")
    logging.info("%s", {'fp_params_dtype': str(fp_params.dtype), 'fp_out_dtype': str(fp_out.dtype)})
    ...
    logging.info("cce raw store and store_fp outputs both match expectations.")
