...
"""Frontend runtime example for pl.move(..., pre_quant_scalar=...) on A5 CCE.

Scalar-dequant counterpart of test_move_fp_cce.py (which uses the per-channel fp_tile path).
This exercises the L0C->UB (Acc->Vec) fixpipe dequant via TMOV with a SCALAR scale, on A5 only
(the L0C->UB quant path lives in arch35/copy_l0c_to_ub_impl.h, gated by PTO_NPU_ARCH_A5; a3 has
no such path).

deqScalar low 32 bits = float32 bit-pattern of the scale (scale=2.0f -> 0x40000000). The signed-int8
flag (bit 46) is re-derived in CCE codegen from the destination tile dtype (MakeBlockOutMoveCodegenCCE),
so the caller passes only the low 32 bits and negatives quantize correctly to signed int8 instead of
saturating to 0.
"""
...
def _make_q(device: str) -> torch.Tensor:
    # Includes negatives on purpose: validates signed-int8 dequant keeps the sign (-3*2=-6, not 0).
...
@pl.jit()
def deq_scalar_move_fp32_to_int8_kernel(
    ...
    with pl.section_cube():
        ...
        # Scalar dequant on the L0C->UB move: TMOV with preQuant scalar (sign flag added in codegen).
...
@pytest.mark.soc("950")
def test_deq_scalar_move_fp32_to_int8():
    device = ST_DEVICE
    ...
    deq_scalar_move_fp32_to_int8_kernel(q, k, move_out)
    ...
    logging.info("***********cce expected move deq_scalar int8 output***********")
    logging.info("%s", expected_out[:8, :8])
    logging.info("***********cce move deq_scalar int8 output (top-left 8x8)***********")
    logging.info("%s", move_out[:8, :8])

    torch.testing.assert_close(move_out.to(torch.int32), expected_out.to(torch.int32), rtol=0, atol=0)
    logging.info("cce move deq_scalar output matches expectation.")
