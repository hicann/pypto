...
def compute_qk(ki, qi, cur_q, q, k, k_l1_db, left_db, right_db, acc_db, qk_vec_db, task_id):
    # --- compute_qk inlined ---
    if ki == 0:
        pl.load_tile(cur_q, q, [qi, 0], order=[1, 0])
    cur_k = k_l1_db.next()
    pl.load_tile(cur_k, k, [ki, 0])
...
def compute_pv(ctx, v_l1_db, p_mat_db, left_db, right_db, acc_db, pv_vec_db, v):
    pl.system.wait_cross_core(
...
def compute_gu(g_ctx, row_off, pv_vec_db, global_sum_rm_buf, exp_corr_rm_fifo, running_o, o_f16, o):
    ...
    if g_ctx.ki == 0:
        ...
        if g_ctx.ki < g_ctx.skv_tiles - 1:
            ...
            pl.store(o, o_f16, [g_ctx.sq_off + row_off, 0])
    pl.system.set_cross_core(
        pipe=pl.PipeType.V,
...
def fa_perf_tkv_preload_dn_kernel(
    ...
    with pl.section_cube():
        ...
        task_id = 0
        ctx_arr = pl.struct_array(4, "CubeCtx", ki=0, task_id_mod2=0, task_id_mod3=0)
        for qi in pl.range(core_id, sq_tiles, num_cores):
            ...
            for ki in pl.range(0, ki_end):
                if ki < skv_tiles:
                    ...
                    compute_qk(ki, qi, cur_q, q, k, k_l1_db, left_db, right_db, acc_db, qk_vec_db, task_id)

                if task_id > 1:
                    prev = ctx_arr[(task_id + 2) % 4]
                    compute_pv(prev, v_l1_db, p_mat_db, left_db, right_db, acc_db, pv_vec_db, v)
                task_id = task_id + 1
    ...
    with pl.section_vector():
        ...
        ctx_arr = pl.struct_array(4, "VecCtx", sq_off=0, ki=0, skv_tiles=0,
            ...
            for ki in pl.range(0, ki_end):
                ...
                if task_id > 0:
                    if ki < skv_tiles + 1:
                        ...
                        compute_p(
                            p_ctx, global_max_rm_buf, global_sum_rm_buf, exp_corr_rm_fifo,
                            qk_vec_db, p_f16_db, p_f16_main_db, p_f16_back_db, p_mat_db, sub_id,
                        )
                if task_id > 2:
                    g_ctx = ctx_arr[(task_id + 1) % 4]
                    compute_gu(g_ctx, row_off, pv_vec_db, global_sum_rm_buf, exp_corr_rm_fifo, running_o, o_f16, o)
                task_id = task_id + 1
            q_count = q_count + 1
...
def make_causal_mask_dn_fixed_2048_u8(device):
    return torch.triu(torch.ones((2048, 2048), dtype=torch.uint8, device=device), diagonal=1)
