...
def alloc_cube_tiles():
    ...
    # DN mode:
    #   QK matmul: K [TKV, TD] x Q^T [TD, TS] -> acc [TKV, TS]
    #   PV matmul: P [TS, TKV] x V  [TKV, TD] -> acc [TS,  TD]
    #
    #   Q loaded with is_transpose=True -> L1 stored as [TD, TS]
...
def compute_qk(
    ...
    if qk_fifo_slot == 0:
        ...
        if l0c_idx == 0:
            pl.set_validshape(qk_vec1, [actual_skv, actual_sq // 2])
...
def compute_pv(
    ...
    # DN: P[TS,TKV] -> Left, V[TKV,TD] -> Right
    ...
    if pv_task_slot == 0:
        if l0c_idx == 0:
            pl.set_validshape(pv_vec, [actual_sq // 2, TD])
        ...
        if l0c_idx == 0:
            pl.set_validshape(pv_vec1, [actual_sq // 2, TD])
...
def softmax_body(
    ...
    # DN: Set UB valid_shape for dynamic buffers (col ops operate on [TKV, TS_HALF])
...
def fa_perf_tkv_preload_dn_kernel(
    ...
    # DN: qk_vec shape [TKV, TS_HALF] (acc dual_mode_split_n result per sub-block)
    qk_vec = pl.make_tile(
        pl.TileType(
            shape=[TKV, TS_HALF],
            dtype=pl.DT_FP32,
        ...
        for qi in pl.range(core_id, sq_tiles, num_cores):
            ...
            for ki in pl.range(0, skv_tiles):
                ...
                compute_qk(
                    ...
                    actual_skv, actual_sq, l0ab_idx, l0c_idx,
    ...
    with pl.section_vector():
        ...
        reduce_dst = pl.make_tile(  # noqa: F841
            pl.TileType(
                shape=[TS_HALF, 1],
                dtype=pl.DT_FP32,
                target_memory=pl.MemorySpace.Vec,
        ...
        ctx_arr = pl.struct_array(3, "VecCtx", sq_off=0, task_id=0, qi=0, ki=0, skv_tiles=0, q_count=0,
                                 core_id=0, sq_dim=0, skv_dim=0, row_off=0, actual_sq=0,
                                 skv_off=0, buf_idx=0, actual_skv=0, actual_sq_half=0)
        ...
        for qi in pl.range(core_id, sq_tiles, num_cores):
            ...
            for ki in pl.range(0, skv_tiles):
                ...
                ctx_curr.sq_dim = sq_dim
                ctx_curr.skv_dim = skv_dim
                ...
                if task_id > 1:
                    ...
                    compute_gu(
                        ...
                        ctx_gu.actual_sq_half, o, gu_tiles,
                    )
                task_id = task_id + 1
            q_count = q_count + 1
