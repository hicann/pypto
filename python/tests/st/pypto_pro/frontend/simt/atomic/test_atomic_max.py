...
"""A5 end-to-end tests for the SIMT atomic_max interface."""
...
def atomic_max_ub_all_dtypes(
    ...
    pl.simt.atomic_max(int32_tile[0, 0], 7)
    pl.simt.atomic_max(uint32_tile[0, 0], 7)
    pl.simt.atomic_max(fp16_tile[0, 0], 7.0)
    pl.simt.atomic_max(bf16_tile[0, 0], 7.0)
...
def simt_atomic_max_ub_all_dtypes(
    ...
    with pl.section_vector():
        ...
        atomic_max_ub_all_dtypes[THREADS](int32_tile, uint32_tile, fp16_tile, bf16_tile, fp32_tile)
...
def atomic_max_gm_all_dtypes(
    ...
    pl.simt.atomic_max(int32_state[0, 0], 7)
    pl.simt.atomic_max(uint32_state[0, 0], 7)
    pl.simt.atomic_max(fp16_state[0, 0], 7.0)
    pl.simt.atomic_max(bf16_state[0, 0], 7.0)
    pl.simt.atomic_max(fp32_state[0, 0], 7.0)
    pl.simt.atomic_max(int64_state[0, 0], 7)
...
def simt_atomic_max_gm_all_dtypes(
    ...
    with pl.section_vector():
        atomic_max_gm_all_dtypes[THREADS](
...
def atomic_max_return_value_gm(
...
def simt_atomic_max_return_value_gm(
    ...
    with pl.section_vector():
        atomic_max_return_value_gm[1](state, old_values)


@pytest.mark.soc("950")
def test_atomic_max_ub_all_supported_dtypes():
    states = [
        torch.full((1, ELEMENTS), 3, dtype=torch.int32),
        torch.full((1, ELEMENTS), 3, dtype=torch.uint32),
        torch.full((1, ELEMENTS), 3, dtype=torch.float16),
        torch.full((1, ELEMENTS), 3, dtype=torch.bfloat16),
        torch.full((1, ELEMENTS), 3, dtype=torch.float32),
    ]
    for state in _run_kernel(simt_atomic_max_ub_all_dtypes, states):
        _assert_target(state, 7)


@pytest.mark.soc("950")
def test_atomic_max_gm_all_supported_dtypes():
    states = [
        torch.full((1, ELEMENTS), 3, dtype=torch.int32),
        torch.full((1, ELEMENTS), 3, dtype=torch.uint32),
        torch.full((1, ELEMENTS), 3, dtype=torch.float16),
        torch.full((1, ELEMENTS), 3, dtype=torch.bfloat16),
        torch.full((1, ELEMENTS), 3, dtype=torch.float32),
        torch.full((1, ELEMENTS), 3, dtype=torch.int64),
        torch.full((1, ELEMENTS), 3, dtype=torch.uint64),
    ]
    for state in _run_kernel(simt_atomic_max_gm_all_dtypes, states):
        _assert_target(state, 7)


@pytest.mark.soc("950")
def test_atomic_max_returns_old_value_and_preserves_other_elements():
    _require_a5()
    state = torch.full((1, ELEMENTS), 123, dtype=torch.int32)
    state[0, 0] = 10
    expected_state = state.clone()
    expected_state[0, 0] = 12
