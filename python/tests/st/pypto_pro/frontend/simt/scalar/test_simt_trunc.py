...
"""A5 generalized system test for the SIMT trunc interface."""
...
def simt_trunc_all_dtypes(
    ...
    with pl.section_vector():
        trunc_all_dtypes[ELEMENTS](src_fp16, src_bf16, src_fp32, out_fp16, out_bf16, out_fp32)


@pytest.mark.soc("950")
def test_trunc_all_supported_dtypes(run_float_unary):
    values = torch.tensor(
        [-2.5000002, -2.5, -2.4999998, -1.5, -0.5, -0.0, 0.0, 0.5, 1.5, 2.4999998, 2.5, 2.5000002]
    ).repeat(6)[:ELEMENTS]
    run_float_unary(simt_trunc_all_dtypes, values, torch.trunc)
