...
"""A5 generalized system test for the SIMT sqrt interface."""
...
def simt_sqrt_all_dtypes(
    ...
    with pl.section_vector():
        sqrt_all_dtypes[ELEMENTS](src_fp16, src_bf16, src_fp32, out_fp16, out_bf16, out_fp32)


@pytest.mark.soc("950")
def test_sqrt_all_supported_dtypes(run_float_unary):
    values = torch.linspace(0.25, 16.0, ELEMENTS)
    values[:5] = torch.tensor([0.0, -0.0, torch.finfo(torch.float32).tiny, 1.0, float("inf")])
    run_float_unary(simt_sqrt_all_dtypes, values, torch.sqrt)
