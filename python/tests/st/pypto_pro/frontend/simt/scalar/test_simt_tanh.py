...
"""A5 generalized system test for the SIMT tanh interface."""
...
def simt_tanh_all_dtypes(
    ...
    with pl.section_vector():
        tanh_all_dtypes[ELEMENTS](src_fp16, src_bf16, src_fp32, out_fp16, out_bf16, out_fp32)


@pytest.mark.soc("950")
def test_tanh_all_supported_dtypes(run_float_unary):
    values = torch.linspace(-4.0, 4.0, ELEMENTS)
    values[:8] = torch.tensor([float("-inf"), -10.0, -1.0e-7, -0.0, 0.0, 1.0e-7, 10.0, float("inf")])
    run_float_unary(simt_tanh_all_dtypes, values, torch.tanh)
