...
"""ASW Matmul (8K-K128 style) — FULLY DYNAMIC / ARBITRARY M/N/K via the 2D pl.move offset.

Dynamic version of ``test_matmul_perf_asw_8k_k128_dn_move_offset.py``: ``a``/``b``/``out``
are all ``[pl.DYNAMIC, pl.DYNAMIC]`` so a single compiled kernel serves any
``[M, K] x [K, N]`` fp16 matmul (the static kernel was hard-wired to M=N=8192, K=128).

The kernel body is identical to ``test_matmul_perf_asw_4k_dn_move_offset_dynamic.py``
(the static 4K and 8K-K128 kernels differed only in their shape constants, now runtime);
this file just exercises the 8K / small-K regime (large M/N, K near 128).

Arbitrary M/N/K:
  * K -- fully arbitrary incl. tails.  ``K_OUTER = ceil(K/KL1)`` wide loads; the last block
    narrows to ``valid_k1 = min(KL1, K - ko*KL1)`` and is split into KL0 sub-tiles by a
    *runtime* inner loop (1 or 2 iters).  The loop (not an unrolled ``if``) keeps the L0
    double-buffer ping-pong balanced so ``auto_mutex`` never deadlocks on the K tail.
    ``phase`` is driven by the global KL0 sub-tile index vs ``last_sub``.
  * M / N -- arbitrary.  The matmul + fixpipe always run on a **full 256x256 tile** (a
    partial fixpipe write/read garbles the tail or hangs); only the GM **loads** clip to
    ``valid_m`` / ``valid_n`` (avoid OOB reads) and the acc gets
    ``set_validshape([valid_m, valid_n])`` right before the **store** (write only the live
    window).  Pad rows/cols come from stale L1 and are computed but never stored; each acc
    element is an independent dot product, so the live window is exact.
...
subject to ``ceil(M/256) % 4 == 0``.

Usage:
    python3 test_matmul_perf_asw_8k_k128_dn_move_offset_dynamic.py
"""
...
def matmul_perf_asw_8k_k128_dn_move_offset_dynamic_kernel(
    ...
    with pl.section_cube():
        ...
        # Runtime tile counts (ceildiv). #M-tiles must be a multiple of MAIN_WINDOW for ASW.
        ...
        for r in pl.range(0, round_cnt):
            ...
            if index < total_cnt:
                ...
                # Boustrophedon (snake) N-walk, written branchless (see 4K variant).
                parity = row_idx % 2
                ...
                for ko in pl.range(0, k_outer):
                    ...
                    # Loads clip to the live window (avoid OOB); the compute stays full-tile.
                    ...
                    # Runtime KL0 sub-tile loop (1 or 2) -> balanced L0 ping-pong on the K tail.
                    n_sub = (valid_k1 + KL0 - 1) // KL0
                    for ki in pl.range(0, n_sub):
                        sub_off = ki * KL0
                        sub_k = pl.min(KL0, valid_k1 - sub_off)
                        gsub = ko * STEP_KA + ki
                ...
                # FIX: ACC -> FP16 GM, clipped to the live valid_m x valid_n window.
...
def _run_case(m, n, k):
    ...
    logging.info(f"\n=== Test ASW Matmul dynamic M={m} N={n} K={k} (8K-K128 regime, arbitrary M/N/K) ===")
    ...
    matmul_perf_asw_8k_k128_dn_move_offset_dynamic_kernel[None, num_cores](a, b, out)
...
def test_matmul_perf_asw_8k_k128_dn_move_offset_dynamic():
    ...
    cases = [
        (2048, 2048, 128),   # K = one wide block (2 KL0 sub-tiles)
        (800, 304, 128),     # M tail 32, N tail 48
        (1024, 300, 96),     # N=300 (unaligned), K<KL1 -> sub-tiles 64 + 32 (K tail)
        (800, 250, 128),     # arbitrary M (tail) and N=250 (unaligned)
        (8192, 8192, 128),   # the original 8K-K128 shape, now via the dynamic kernel
    ]
    for m, n, k in cases:
        _run_case(m, n, k)
...
if __name__ == "__main__":
    logging.info("ASW Matmul 8K-K128 move-offset Test (fully dynamic / arbitrary M/N/K)")
    logging.info("=" * 60)
    test_matmul_perf_asw_8k_k128_dn_move_offset_dynamic()
    logging.info("\nTest completed!")
