...
def test_pow_onboard():
        ...
        for b_idx in pypto.loop(b_loop_num, name="b0", idx_name="bidx"):
            for s_idx in pypto.loop(s_loop_num, name="s0", idx_name="sidx"):
                view_tensor_a = pypto.view(
                    input1, view_shape, [b_idx * view_shape[0], s_idx * view_shape[1]],
                    valid_shape=[
                ...
                view_tensor_b = pypto.view(
                    input2, view_shape, [b_idx * view_shape[0], s_idx * view_shape[1]],
                    valid_shape=[
                        pypto.min(pypto.symbolic_scalar(shape[0]) - b_idx * view_shape[0],
                                pypto.symbolic_scalar(view_shape[0])),
                        pypto.min(pypto.symbolic_scalar(shape[1]) - s_idx * view_shape[1],
                ...
                result = pypto.pow(view_tensor_a, view_tensor_b)
                pypto.assemble(
                    result, [b_idx * view_shape[0], s_idx * view_shape[1]], output)

    assert isinstance(output, pypto.tensor)

    a_tensor = torch.randint(low=-10, high=10, size=[shape[0], shape[1]], dtype=torch.float32)
    b_tensor = torch.randint(low=-4, high=4, size=[shape[0], shape[1]], dtype=torch.float32)
    ...
    golden = torch.pow(a_tensor, b_tensor)
