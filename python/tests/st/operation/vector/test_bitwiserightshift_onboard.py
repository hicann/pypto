...
def test_bitwiserightshift_onboard():
    ...
    with pypto.function("MAIN", input1, input2, output):
        for b_idx in pypto.loop(b_loop_num, name="b0", idx_name="bidx"):
            for s_idx in pypto.loop(s_loop_num, name="s0", idx_name="sidx"):
                ...
                view_tensor_a = pypto.bitwise_right_shift(view_tensor_a, view_tensor_b)
                pypto.assemble(view_tensor_a, [b_idx * view_shape[0], s_idx * view_shape[1]], output)

    assert isinstance(output, pypto.tensor)

    a_tensor = torch.randint(low=-100, high=100, size=[shape[0], shape[1]], dtype=torch.int16)
    b_tensor = torch.randint(low=-100, high=100, size=[shape[0], shape[1]], dtype=torch.int16)
    out_tensor = torch.zeros(shape[0], shape[1], dtype=torch.int16)
    pto_a_tensor = pypto.from_torch(a_tensor, "a_tensor")
    pto_b_tensor = pypto.from_torch(b_tensor, "b_tensor")
    pto_out_tensor = pypto.from_torch(out_tensor, "b_tensor")
    pypto.runtime._device_run_once_data_from_host(pto_a_tensor, pto_b_tensor, pto_out_tensor)

    golden = torch.bitwise_right_shift(a_tensor, b_tensor)
