...
"""System tests for ASinh migrated from the C++ vector ST."""
...
from vector_testcase.asinh_onboard_test_case import ASINH_ONBOARD_TESTS, AsinhOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def asinh_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AsinhOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.asinh(input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): asinh_onboard_2d_1input_kernel,
}


def run_asinh_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AsinhOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.asinh(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", ASINH_ONBOARD_TESTS, ids=[case["case_name"] for case in ASINH_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_asinh_onboard(case: dict):
    run_asinh_onboard_test(case)
