...
"""System tests for Erf migrated from the C++ vector ST."""
...
from vector_testcase.erf_onboard_test_case import ERF_ONBOARD_TESTS, ErfOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def erf_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: ErfOnboardConfig):
...
KERNELS = {
    (2, 1): erf_onboard_2d_1input_kernel,
}


def run_erf_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ErfOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.erf(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", ERF_ONBOARD_TESTS, ids=[case["case_name"] for case in ERF_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_erf_onboard(case: dict):
    run_erf_onboard_test(case)
