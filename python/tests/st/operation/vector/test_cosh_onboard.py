...
"""System tests for Cosh migrated from the C++ vector ST."""
...
from vector_testcase.cosh_onboard_test_case import COSH_ONBOARD_TESTS, CoshOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def cosh_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: CoshOnboardConfig):
...
KERNELS = {
    (2, 1): cosh_onboard_2d_1input_kernel,
}


def run_cosh_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = CoshOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.cosh(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", COSH_ONBOARD_TESTS, ids=[case["case_name"] for case in COSH_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_cosh_onboard(case: dict):
    run_cosh_onboard_test(case)
