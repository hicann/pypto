...
"""System tests for BitwiseLeftShift migrated from the C++ vector ST."""
...
from vector_testcase.bitwiseleftshift_onboard_test_case import (
    BITWISELEFTSHIFT_ONBOARD_TESTS,
    BitwiseleftshiftOnboardConfig,
)
...
def bitwiseleftshift_onboard_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: BitwiseleftshiftOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_left_shift(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): bitwiseleftshift_onboard_2d_2input_kernel,
}


def run_bitwiseleftshift_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwiseleftshiftOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_left_shift(*inputs_cpu)]
