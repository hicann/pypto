...
"""System tests for Sum migrated from the C++ vector ST."""
...
from vector_testcase.sum_onboard_test_case import SUM_ONBOARD_TESTS, SumOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def sum_onboard_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: SumOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.sum(input0_view, config.dims[0], config.keep_dim)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def sum_onboard_3d_1input_3d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: SumOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 1, 1, 2): sum_onboard_2d_1input_2d_output_kernel,
    (3, 1, 1, 3): sum_onboard_3d_1input_3d_output_kernel,
}


def run_sum_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SumOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.sum(inputs_cpu[0], dim=config.dims[0], keepdim=config.keep_dim)]
...
@pytest.mark.parametrize("case", SUM_ONBOARD_TESTS, ids=[case["case_name"] for case in SUM_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_sum_onboard(case: dict):
    run_sum_onboard_test(case)
