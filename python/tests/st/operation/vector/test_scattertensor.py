...
"""System tests for ScatterTensor migrated from the C++ vector ST."""
...
from vector_testcase.scattertensor_test_case import SCATTERTENSOR_TESTS, ScattertensorConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def scattertensor_2d_3input_2d_output_kernel(
    input0: pypto.Tensor(),
    input1: pypto.Tensor(),
    input2: pypto.Tensor(),
    output0: pypto.Tensor(),
    config: ScattertensorConfig,
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            input2_offset = [0 if config.input_shapes[2][axis] == 1 else offsets[axis] for axis in range(2)]
            input2_view = pypto.view(input2, config.input_view_shapes[2], input2_offset)
            result = pypto.scatter_(input0_view, config.axis, input1_view, input2_view, reduce=config.reduce or None)
            output_offset = [
...
KERNELS = {
    (2, 3, 1, 2): scattertensor_2d_3input_2d_output_kernel,
}


def run_scattertensor_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ScattertensorConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = (
        [inputs_cpu[0].scatter_add(config.axis, inputs_cpu[1].long(), inputs_cpu[2])]
        if config.reduce == 'add'
        else [inputs_cpu[0].scatter(config.axis, inputs_cpu[1].long(), inputs_cpu[2])]
    )
