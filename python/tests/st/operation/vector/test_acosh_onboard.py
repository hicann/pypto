...
"""System tests for ACosh migrated from the C++ vector ST."""
...
from vector_testcase.acosh_onboard_test_case import ACOSH_ONBOARD_TESTS, AcoshOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def acosh_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AcoshOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.acosh(input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): acosh_onboard_2d_1input_kernel,
}


def run_acosh_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AcoshOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.acosh(inputs_cpu[0])]
