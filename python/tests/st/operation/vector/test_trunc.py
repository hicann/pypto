...
"""System tests for Trunc migrated from the C++ vector ST."""
...
from vector_testcase.trunc_test_case import TRUNC_TESTS, TruncConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def trunc_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: TruncConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.trunc(input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): trunc_2d_1input_kernel,
}


def run_trunc_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = TruncConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.trunc(inputs_cpu[0])]
