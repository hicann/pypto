...
"""System tests for Amax migrated from the C++ vector ST."""
...
from vector_testcase.amax_onboard_test_case import AMAX_ONBOARD_TESTS, AmaxOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def amax_onboard_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: AmaxOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.amax(input0_view, config.dims[0], config.keep_dim)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def amax_onboard_3d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: AmaxOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def amax_onboard_3d_1input_3d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: AmaxOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def amax_onboard_4d_1input_4d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: AmaxOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
...
KERNELS = {
    (2, 1, 1, 2): amax_onboard_2d_1input_2d_output_kernel,
    (3, 1, 1, 2): amax_onboard_3d_1input_2d_output_kernel,
    (3, 1, 1, 3): amax_onboard_3d_1input_3d_output_kernel,
    (4, 1, 1, 4): amax_onboard_4d_1input_4d_output_kernel,
}


def run_amax_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AmaxOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.amax(inputs_cpu[0], dim=config.dims[0], keepdim=config.keep_dim)]
