...
"""System tests for Asin migrated from the C++ vector ST."""
...
from vector_testcase.asin_test_case import ASIN_TESTS, AsinConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def asin_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AsinConfig):
...
KERNELS = {
    (2, 1): asin_2d_1input_kernel,
}


def run_asin_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AsinConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.asin(inputs_cpu[0])]
