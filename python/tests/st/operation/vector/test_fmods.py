...
"""System tests for Fmods migrated from the C++ vector ST."""
...
from vector_testcase.fmods_test_case import FMODS_TESTS, FmodsConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def fmods_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: FmodsConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.fmod(input0_view, config.scalar)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def fmods_4d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: FmodsConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 1): fmods_2d_1input_kernel,
    (4, 1): fmods_4d_1input_kernel,
}


def run_fmods_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = FmodsConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.fmod(inputs_cpu[0], config.scalar)]
