...
"""System tests for Divs migrated from the C++ vector ST."""
...
from vector_testcase.divs_onboard_test_case import DIVS_ONBOARD_TESTS, DivsOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def divs_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: DivsOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.div(input0_view, config.scalar)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def divs_onboard_3d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: DivsOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def divs_onboard_4d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: DivsOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
...
KERNELS = {
    (2, 1): divs_onboard_2d_1input_kernel,
    (3, 1): divs_onboard_3d_1input_kernel,
    (4, 1): divs_onboard_4d_1input_kernel,
}


def run_divs_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = DivsOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.div(inputs_cpu[0], config.scalar)]
