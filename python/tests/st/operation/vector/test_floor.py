...
"""System tests for Floor migrated from the C++ vector ST."""
...
from vector_testcase.floor_test_case import FLOOR_TESTS, FloorConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def floor_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: FloorConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.floor(input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): floor_2d_1input_kernel,
}


def run_floor_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = FloorConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.floor(inputs_cpu[0])]
