...
"""System tests for Compare migrated from the C++ vector ST."""
...
from vector_testcase.compare_test_case import COMPARE_TESTS, CompareConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def compare_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: CompareConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.eq(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): compare_2d_2input_kernel,
    (4, 2): compare_4d_2input_kernel,
}


def run_compare_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = CompareConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.eq(inputs_cpu[0], inputs_cpu[1])]
