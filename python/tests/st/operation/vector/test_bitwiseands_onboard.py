...
"""System tests for BitwiseAnds migrated from the C++ vector ST."""
...
from vector_testcase.bitwiseands_onboard_test_case import BITWISEANDS_ONBOARD_TESTS, BitwiseandsOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def bitwiseands_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: BitwiseandsOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_and(input0_view, config.scalar)
            output_offset = [
...
KERNELS = {
    (2, 1): bitwiseands_onboard_2d_1input_kernel,
}


def run_bitwiseands_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwiseandsOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_and(inputs_cpu[0], config.scalar)]
...
@pytest.mark.parametrize(
    "case", BITWISEANDS_ONBOARD_TESTS, ids=[case["case_name"] for case in BITWISEANDS_ONBOARD_TESTS]
)
@pypto.options(pass_options={"enable_slice": True})
def test_bitwiseands_onboard(case: dict):
    run_bitwiseands_onboard_test(case)
