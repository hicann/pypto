...
"""System tests for Permute migrated from the C++ vector ST."""
...
from vector_testcase.permute_test_case import PERMUTE_TESTS, PermuteConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def permute_3d_1input_3d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: PermuteConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
                ...
                input0_view = input0[:]
                result = pypto.permute(input0_view, config.perm)
                output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def permute_4d_1input_4d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: PermuteConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def permute_5d_1input_5d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: PermuteConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
                for index_3 in pypto.loop(config.loop_ranges[3]):
                    for index_4 in pypto.loop(config.loop_ranges[4]):
                        offsets = [
                            index_0 * config.execution_view_shape[0],
                            index_1 * config.execution_view_shape[1],
                            index_2 * config.execution_view_shape[2],
                            index_3 * config.execution_view_shape[3],
                            index_4 * config.execution_view_shape[4],
                        ]
                        input0_view = input0[:]
                        result = pypto.permute(input0_view, config.perm)
                        output_offset = [
...
KERNELS = {
    (3, 1, 1, 3): permute_3d_1input_3d_output_kernel,
    (4, 1, 1, 4): permute_4d_1input_4d_output_kernel,
    (5, 1, 1, 5): permute_5d_1input_5d_output_kernel,
}


def run_permute_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = PermuteConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.permute(inputs_cpu[0], config.perm)]
