...
"""System tests for Exp migrated from the C++ vector ST."""
...
from vector_testcase.exp_test_case import EXP_TESTS, ExpConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def exp_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: ExpConfig):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def exp_3d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: ExpConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 1): exp_2d_1input_kernel,
    (3, 1): exp_3d_1input_kernel,
}


def run_exp_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ExpConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.exp(inputs_cpu[0])]
