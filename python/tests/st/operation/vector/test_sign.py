...
"""System tests for Sign migrated from the C++ vector ST."""
...
from vector_testcase.sign_test_case import SIGN_TESTS, SignConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def sign_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: SignConfig):
...
KERNELS = {
    (2, 1): sign_2d_1input_kernel,
}


def run_sign_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SignConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.sign(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", SIGN_TESTS, ids=[case["case_name"] for case in SIGN_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_sign(case: dict):
    run_sign_test(case)
