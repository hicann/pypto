...
"""System tests for Ceil migrated from the C++ vector ST."""
...
from vector_testcase.ceil_test_case import CEIL_TESTS, CeilConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def ceil_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: CeilConfig):
...
KERNELS = {
    (2, 1): ceil_2d_1input_kernel,
}


def run_ceil_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = CeilConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.ceil(inputs_cpu[0])]
