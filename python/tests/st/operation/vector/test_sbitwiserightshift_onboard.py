...
"""System tests for SBitwiseRightShift migrated from the C++ vector ST."""
...
from vector_testcase.sbitwiserightshift_onboard_test_case import (
    SBITWISERIGHTSHIFT_ONBOARD_TESTS,
    SbitwiserightshiftOnboardConfig,
)
...
def sbitwiserightshift_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: SbitwiserightshiftOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_right_shift(config.scalar, input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): sbitwiserightshift_onboard_2d_1input_kernel,
}


def run_sbitwiserightshift_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SbitwiserightshiftOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_right_shift(config.scalar, inputs_cpu[0])]
