...
"""System tests for BitwiseXors migrated from the C++ vector ST."""
...
from vector_testcase.bitwisexors_onboard_test_case import BITWISEXORS_ONBOARD_TESTS, BitwisexorsOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def bitwisexors_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: BitwisexorsOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_xor(input0_view, config.scalar)
            output_offset = [
...
KERNELS = {
    (2, 1): bitwisexors_onboard_2d_1input_kernel,
}


def run_bitwisexors_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwisexorsOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_xor(inputs_cpu[0], config.scalar)]
