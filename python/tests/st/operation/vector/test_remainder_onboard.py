...
"""System tests for Remainder migrated from the C++ vector ST."""
...
from vector_testcase.remainder_onboard_test_case import REMAINDER_ONBOARD_TESTS, RemainderOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def remainder_onboard_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: RemainderOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.remainder(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): remainder_onboard_2d_2input_kernel,
}


def run_remainder_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = RemainderOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.remainder(*inputs_cpu)]
...
@pytest.mark.parametrize("case", REMAINDER_ONBOARD_TESTS, ids=[case["case_name"] for case in REMAINDER_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_remainder_onboard(case: dict):
    run_remainder_onboard_test(case)
