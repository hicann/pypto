...
"""System tests for BitwiseAnd migrated from the C++ vector ST."""
...
from vector_testcase.bitwiseand_onboard_test_case import BITWISEAND_ONBOARD_TESTS, BitwiseandOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def bitwiseand_onboard_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: BitwiseandOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_and(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): bitwiseand_onboard_2d_2input_kernel,
}


def run_bitwiseand_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwiseandOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_and(*inputs_cpu)]
