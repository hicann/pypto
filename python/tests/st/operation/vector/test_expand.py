...
"""System tests for Expand migrated from the C++ vector ST."""
...
from vector_testcase.expand_test_case import EXPAND_TESTS, ExpandConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def expand_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: ExpandConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.expand_clone(input0_view, config.view_shape)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def expand_4d_1input_4d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: ExpandConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 1, 1, 2): expand_2d_1input_2d_output_kernel,
    (4, 1, 1, 4): expand_4d_1input_4d_output_kernel,
}


def run_expand_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ExpandConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [inputs_cpu[0].expand(config.output_shapes[0]).clone()]
