...
"""System tests for BitwiseRightShifts migrated from the C++ vector ST."""
...
from vector_testcase.bitwiserightshifts_onboard_test_case import (
    BITWISERIGHTSHIFTS_ONBOARD_TESTS,
    BitwiserightshiftsOnboardConfig,
)
...
def bitwiserightshifts_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: BitwiserightshiftsOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_right_shift(input0_view, config.scalar)
            output_offset = [
...
KERNELS = {
    (2, 1): bitwiserightshifts_onboard_2d_1input_kernel,
}


def run_bitwiserightshifts_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwiserightshiftsOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_right_shift(inputs_cpu[0], config.scalar)]
