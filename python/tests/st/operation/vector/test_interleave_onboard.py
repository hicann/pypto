...
def test_tensor_interleave():
    ...
    with pypto.function("MAIN", input0, input1, output0, output1):
        for b_idx in pypto.loop(b_loop_num, name="b0", idx_name="bidx"):
            for s_idx in pypto.loop(s_loop_num, name="s0", idx_name="sidx"):
                ...
                result0, result1 = pypto.interleave(view_tensor0, view_tensor1)
    ...
    interleaved = torch.stack((input0_tensor, input1_tensor), dim=-1).flatten(-2)
    golden0 = interleaved[..., :shape[-1]]
    golden1 = interleaved[..., shape[-1]:]
