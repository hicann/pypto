...
"""System tests for Sqrt migrated from the C++ vector ST."""
...
from vector_testcase.sqrt_test_case import SQRT_TESTS, SqrtConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def sqrt_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: SqrtConfig):
...
KERNELS = {
    (2, 1): sqrt_2d_1input_kernel,
}


def run_sqrt_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SqrtConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.sqrt(inputs_cpu[0])]
