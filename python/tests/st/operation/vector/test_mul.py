...
"""System tests for Mul migrated from the C++ vector ST."""
...
from vector_testcase.mul_test_case import MUL_TESTS, MulConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def mul_2d_2input_kernel(input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: MulConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.mul(input0_view, input1_view)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def mul_4d_2input_kernel(input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: MulConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 2): mul_2d_2input_kernel,
    (4, 2): mul_4d_2input_kernel,
}


def run_mul_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = MulConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.mul(*inputs_cpu)]
