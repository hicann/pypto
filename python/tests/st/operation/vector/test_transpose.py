...
"""System tests for Transpose migrated from the C++ vector ST."""
...
from vector_testcase.transpose_test_case import TRANSPOSE_TESTS, TransposeConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def transpose_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: TransposeConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.transpose(input0_view, config.first_dim, config.second_dim)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def transpose_3d_1input_3d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: TransposeConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def transpose_4d_1input_4d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: TransposeConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
...
KERNELS = {
    (2, 1, 1, 2): transpose_2d_1input_2d_output_kernel,
    (3, 1, 1, 3): transpose_3d_1input_3d_output_kernel,
    (4, 1, 1, 4): transpose_4d_1input_4d_output_kernel,
}


def run_transpose_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = TransposeConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.transpose(inputs_cpu[0], config.first_dim, config.second_dim)]
