...
"""System tests for Sin migrated from the C++ vector ST."""
...
from vector_testcase.sin_onboard_test_case import SIN_ONBOARD_TESTS, SinOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def sin_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: SinOnboardConfig):
...
KERNELS = {
    (2, 1): sin_onboard_2d_1input_kernel,
}


def run_sin_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SinOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.sin(inputs_cpu[0])]
