...
def op_tanh(params, a, b):
    n, m = a.shape
    view_shape, tile_shape = params
    for b_idx in pypto.loop(int(np.ceil(n / view_shape[0])), name="LOOP_TANH_L0", idx_name="b_idx"):
        for s_idx in pypto.loop(int(np.ceil(m / view_shape[1])), name="LOOP_TANH_L1", idx_name="s_idx"):
            tile_a = pypto.view(
                a,
                view_shape,
            ...
            tile_a.move(pypto.tanh(tile_a))
            pypto.assemble(tile_a, [b_idx * view_shape[0], s_idx * view_shape[1]], b)


def op_tanh_golden(param, a, b):
    return torch.tanh(a)


class TanhTest(TestBuilder):
    ...
    def get_input_from_param(self):
        ...
        a_tensor = torch.rand(n, m, dtype=torch.float32) * 10 - 5
...
def test():
    st = TanhTest(((16, 16), (8, 8)), op_tanh, op_tanh_golden, tiling=32)
    st()
