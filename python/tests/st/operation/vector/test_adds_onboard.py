...
"""System tests for Adds migrated from the C++ vector ST."""
...
from vector_testcase.adds_onboard_test_case import ADDS_ONBOARD_TESTS, AddsOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def adds_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AddsOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.add(input0_view, config.scalar)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def adds_onboard_4d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AddsOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 1): adds_onboard_2d_1input_kernel,
    (4, 1): adds_onboard_4d_1input_kernel,
}


def run_adds_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AddsOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.add(inputs_cpu[0], config.scalar)]
