...
"""System tests for Acos migrated from the C++ vector ST."""
...
from vector_testcase.acos_test_case import ACOS_TESTS, AcosConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def acos_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AcosConfig):
...
KERNELS = {
    (2, 1): acos_2d_1input_kernel,
}


def run_acos_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AcosConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.acos(inputs_cpu[0])]
