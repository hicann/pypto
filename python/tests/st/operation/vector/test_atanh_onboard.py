...
"""System tests for Atanh migrated from the C++ vector ST."""
...
from vector_testcase.atanh_onboard_test_case import ATANH_ONBOARD_TESTS, AtanhOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def atanh_onboard_1d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AtanhOnboardConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        ...
        result = pypto.atanh(input0_view)
        output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def atanh_onboard_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: AtanhOnboardConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
...
KERNELS = {
    (1, 1): atanh_onboard_1d_1input_kernel,
    (2, 1): atanh_onboard_2d_1input_kernel,
}


def run_atanh_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = AtanhOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.atanh(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", ATANH_ONBOARD_TESTS, ids=[case["case_name"] for case in ATANH_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_atanh_onboard(case: dict):
    run_atanh_onboard_test(case)
