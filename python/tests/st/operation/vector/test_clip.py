...
"""System tests for Clip migrated from the C++ vector ST."""
...
from vector_testcase.clip_test_case import CLIP_TESTS, ClipConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def clip_3d_3input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), input2: pypto.Tensor(), output: pypto.Tensor(), config: ClipConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            for index_2 in pypto.loop(config.loop_ranges[2]):
                ...
                input2_offset = [0 if config.input_shapes[2][axis] == 1 else offsets[axis] for axis in range(3)]
                input2_view = pypto.view(input2, config.input_view_shapes[2], input2_offset)
                result = pypto.clip(input0_view, input1_view, input2_view)
                output_offset = [
...
KERNELS = {
    (3, 3): clip_3d_3input_kernel,
}


def run_clip_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ClipConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.clamp(inputs_cpu[0], min=inputs_cpu[1], max=inputs_cpu[2])]
