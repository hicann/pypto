...
"""System tests for Pows migrated from the C++ vector ST."""
...
from vector_testcase.pows_test_case import POWS_TESTS, PowsConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def pows_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: PowsConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.pow(input0_view, config.scalar)
            output_offset = [
...
KERNELS = {
    (2, 1): pows_2d_1input_kernel,
}


def run_pows_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = PowsConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.pow(inputs_cpu[0], config.scalar)]
