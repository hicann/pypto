...
"""System tests for Concat migrated from the C++ vector ST."""
...
from vector_testcase.concat_test_case import CONCAT_TESTS, ConcatConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def concat_2d_2input_2d_output_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output0: pypto.Tensor(), config: ConcatConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.concat([input0_view, input1_view], config.axis)
            output_offset = [
...
KERNELS = {
    (2, 2, 1, 2): concat_2d_2input_2d_output_kernel,
}


def run_concat_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = ConcatConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.concat(inputs_cpu, dim=config.axis)]
