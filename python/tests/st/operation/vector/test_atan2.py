...
"""System tests for Atan2 migrated from the C++ vector ST."""
...
from vector_testcase.atan2_test_case import ATAN2_TESTS, Atan2Config

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def atan2_2d_2input_kernel(input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: Atan2Config):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.atan2(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): atan2_2d_2input_kernel,
}


def run_atan2_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = Atan2Config.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.atan2(*inputs_cpu)]
