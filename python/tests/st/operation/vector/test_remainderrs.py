...
"""System tests for RemainderRS migrated from the C++ vector ST."""
...
from vector_testcase.remainderrs_test_case import REMAINDERRS_TESTS, RemainderrsConfig
...
def remainderrs_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: RemainderrsConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.remainder(config.scalar, input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): remainderrs_2d_1input_kernel,
}


def run_remainderrs_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = RemainderrsConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.remainder(config.scalar, inputs_cpu[0]).to(TORCH_DTYPES[config.output_tensors[0].dtype])]
...
@pytest.mark.parametrize("case", REMAINDERRS_TESTS, ids=[case["case_name"] for case in REMAINDERRS_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_remainderrs(case: dict):
    run_remainderrs_test(case)
