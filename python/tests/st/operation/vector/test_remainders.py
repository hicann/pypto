...
"""System tests for RemainderS migrated from the C++ vector ST."""
...
from vector_testcase.remainders_test_case import REMAINDERS_TESTS, RemaindersConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def remainders_2d_1input_kernel(input0: pypto.Tensor(), output: pypto.Tensor(), config: RemaindersConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.remainder(input0_view, config.scalar)
            output_offset = [
...
KERNELS = {
    (2, 1): remainders_2d_1input_kernel,
}


def run_remainders_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = RemaindersConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.remainder(inputs_cpu[0], config.scalar)]
