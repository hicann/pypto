...
"""System tests for BitwiseNot migrated from the C++ vector ST."""
...
from vector_testcase.bitwisenot_onboard_test_case import BITWISENOT_ONBOARD_TESTS, BitwisenotOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def bitwisenot_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: BitwisenotOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_not(input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): bitwisenot_onboard_2d_1input_kernel,
}


def run_bitwisenot_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwisenotOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_not(inputs_cpu[0])]
...
@pytest.mark.parametrize("case", BITWISENOT_ONBOARD_TESTS, ids=[case["case_name"] for case in BITWISENOT_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_bitwisenot_onboard(case: dict):
    run_bitwisenot_onboard_test(case)
