...
"""System tests for BitwiseXor migrated from the C++ vector ST."""
...
from vector_testcase.bitwisexor_onboard_test_case import BITWISEXOR_ONBOARD_TESTS, BitwisexorOnboardConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def bitwisexor_onboard_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: BitwisexorOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_xor(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): bitwisexor_onboard_2d_2input_kernel,
}


def run_bitwisexor_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = BitwisexorOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_xor(*inputs_cpu)]
...
@pytest.mark.parametrize("case", BITWISEXOR_ONBOARD_TESTS, ids=[case["case_name"] for case in BITWISEXOR_ONBOARD_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_bitwisexor_onboard(case: dict):
    run_bitwisexor_onboard_test(case)
