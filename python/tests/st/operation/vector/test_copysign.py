...
"""System tests for CopySign migrated from the C++ vector ST."""
...
from vector_testcase.copysign_test_case import COPYSIGN_TESTS, CopysignConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def copysign_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: CopysignConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.copysign(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): copysign_2d_2input_kernel,
}


def run_copysign_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = CopysignConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.copysign(*inputs_cpu)]
