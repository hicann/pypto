...
"""System tests for CumProd migrated from the C++ vector ST."""
...
from vector_testcase.cumprod_test_case import CUMPROD_TESTS, CumprodConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def cumprod_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: CumprodConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.cumprod(input0_view, config.axis)
            output_offset = [
...
KERNELS = {
    (2, 1, 1, 2): cumprod_2d_1input_2d_output_kernel,
}


def run_cumprod_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = CumprodConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.cumprod(inputs_cpu[0], dim=config.axis)]
