...
"""System tests for Div migrated from the C++ vector ST."""
...
from vector_testcase.div_test_case import DIV_TESTS, DivConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def div_2d_2input_kernel(input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: DivConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.div(input0_view, input1_view)
            output_offset = [
...
@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def div_3d_2input_kernel(input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: DivConfig):
    pypto.set_vec_tile_shapes(*config.tile_shape)
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
...
KERNELS = {
    (2, 2): div_2d_2input_kernel,
    (3, 2): div_3d_2input_kernel,
}


def run_div_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = DivConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.div(*inputs_cpu)]
