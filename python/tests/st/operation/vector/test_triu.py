...
"""System tests for TriU migrated from the C++ vector ST."""
...
from vector_testcase.triu_test_case import TRIU_TESTS, TriuConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def triu_2d_1input_2d_output_kernel(input0: pypto.Tensor(), output0: pypto.Tensor(), config: TriuConfig):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.triu(input0_view, config.diagonal)
            output_offset = [
...
KERNELS = {
    (2, 1, 1, 2): triu_2d_1input_2d_output_kernel,
}


def run_triu_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = TriuConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.triu(inputs_cpu[0], config.diagonal)]
...
@pytest.mark.parametrize("case", TRIU_TESTS, ids=[case["case_name"] for case in TRIU_TESTS])
@pypto.options(pass_options={"enable_slice": True})
def test_triu(case: dict):
    run_triu_test(case)
