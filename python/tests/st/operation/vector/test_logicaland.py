...
"""System tests for LogicalAnd migrated from the C++ vector ST."""
...
from vector_testcase.logicaland_test_case import LOGICALAND_TESTS, LogicalandConfig

import pypto


@pypto.frontend.jit(debug_options={"runtime_debug_mode": 0, "compile_debug_mode": 0})
def logicaland_2d_2input_kernel(
    input0: pypto.Tensor(), input1: pypto.Tensor(), output: pypto.Tensor(), config: LogicalandConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.logical_and(input0_view, input1_view)
            output_offset = [
...
KERNELS = {
    (2, 2): logicaland_2d_2input_kernel,
}


def run_logicaland_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = LogicalandConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.logical_and(*inputs_cpu)]
