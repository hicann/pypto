...
"""System tests for SBitwiseLeftShift migrated from the C++ vector ST."""
...
from vector_testcase.sbitwiseleftshift_onboard_test_case import (
    SBITWISELEFTSHIFT_ONBOARD_TESTS,
    SbitwiseleftshiftOnboardConfig,
)
...
def sbitwiseleftshift_onboard_2d_1input_kernel(
    input0: pypto.Tensor(), output: pypto.Tensor(), config: SbitwiseleftshiftOnboardConfig
):
    ...
    for index_0 in pypto.loop(config.loop_ranges[0]):
        for index_1 in pypto.loop(config.loop_ranges[1]):
            ...
            result = pypto.bitwise_left_shift(config.scalar, input0_view)
            output_offset = [
...
KERNELS = {
    (2, 1): sbitwiseleftshift_onboard_2d_1input_kernel,
}


def run_sbitwiseleftshift_onboard_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    config = SbitwiseleftshiftOnboardConfig.from_test_case(case)
    inputs_cpu = make_inputs(config)
    expected = [torch.bitwise_left_shift(config.scalar, inputs_cpu[0])]
...
@pytest.mark.parametrize(
    "case", SBITWISELEFTSHIFT_ONBOARD_TESTS, ids=[case["case_name"] for case in SBITWISELEFTSHIFT_ONBOARD_TESTS]
)
@pypto.options(pass_options={"enable_slice": True})
def test_sbitwiseleftshift_onboard(case: dict):
    run_sbitwiseleftshift_onboard_test(case)
