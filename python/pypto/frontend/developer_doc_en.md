...
### Function-level assumption table

`pypto.experimental.assume_divisible(expr, divisor)` returns a `SymbolicScalar` representing `expr`; `SymbolicScalar` inputs are returned unchanged, while `int` inputs are converted. It also records a compile-time assumption on the Program. The table stores `expr.Simplify()` and uses its `Dump()` as the key, so the stored expression and key have the same normalized form and equivalent expressions share an assumption. All Functions in the same Program can query the assumption.

`InferDynShape` consumes the assumption before static valid-shape snapshots are recorded. This lets later passes such as dualdst use the static path while preserving dynamic valid shapes on non-split axes. By default, the API is an unchecked contract. With `runtime_debug_mode=4`, codegen lowers every explicitly registered, normalized assumption expression to a modulo assertion at the control-flow entry; generated code evaluates the expression using actual runtime values. Compiler-derived expressions that were not explicitly registered are not checked.
