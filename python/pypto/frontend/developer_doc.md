...
`pypto.experimental.assume_divisible(expr, divisor)` 返回表示 `expr` 的 `SymbolicScalar`；`SymbolicScalar` 输入原样返回，`int` 输入会转换为 `SymbolicScalar`。同时，接口在 Program 上登记编译期假设。假设表保存 `expr.Simplify()`，并使用其 `Dump()` 作为 key，因此存储表达式与 key 具有相同的规范化形式，化简后等价的表达式会合并为同一条假设。同一 Program 中的所有 Function 均可查询该假设。

`InferDynShape` 在记录静态 valid shape 快照前消费该信息，帮助化简动态 valid shape；后续 dualdst 等 pass 可据此走静态路径，同时非切分轴的动态 valid shape 仍按原样携带。默认情况下，该 API 是无运行时检查的语义契约。开启 `runtime_debug_mode=4` 时，codegen 会将每个由用户显式登记并规范化后的假设表达式转换为控制流入口处的取模断言，生成代码使用实际运行时值计算该表达式；未通过 `assume_divisible` 显式登记的编译器推导表达式不检查。
