...
def scan_buffer_addr_ranges(kernel_func_def: ast.FunctionDef,
...
def scan_kernel_slot_to_buffer(func_def: ast.FunctionDef,
                               cross_buffers: dict) -> dict[str, str]:
...
def build_binding_map(func_def: ast.FunctionDef, call_args: list | None,
                      caller_bindings: dict, kernel_slot_map: dict,
                      cross_buffers: dict) -> dict[str, tuple[str, bool]]:
...
def _build_slot_to_buffer_from_bindings(func_def: ast.FunctionDef,
                                        bindings: dict[str, tuple[str, bool]]) -> dict[str, str]:
...
def _scan_accesses_recursive(
    ...
    for node in _iter_calls_in_order(func_def):
        ...
        if isinstance(node.func, ast.Name) and node.func.id in stage_func_defs:
            ...
            sub_bindings = build_binding_map(
                sub_def, node.args, bindings, kernel_slot_map, cross_buffers)
            sub_slot_to_buffer = _build_slot_to_buffer_from_bindings(sub_def, sub_bindings)
            _scan_accesses_recursive(
                sub_def, sub_slot_to_buffer, cross_buffers, all_buffer_memory,
                vf_func_defs, cross_access, overlap_map, local_access,
                sub_bindings, kernel_slot_map, stage_func_defs)
            continue
        # pl.<op>(...) block op
        if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) \
                and node.func.value.id == "pl":
            _handle_block_op(node, slot_to_buffer, cross_buffers, all_buffer_memory,
                             cross_access, overlap_map, local_access)
        # VF helper call
        elif isinstance(node.func, ast.Name):
            _handle_vf_call(node, slot_to_buffer, cross_buffers, vf_func_defs, cross_access)


def _handle_block_op(node: ast.Call, slot_to_buffer, cross_buffers, all_buffer_memory,
                     cross_access, overlap_map=None, local_access=None):
...
def scan_stage_accesses(stage_func_def: ast.FunctionDef,
                        ...
                        stage_func_defs: dict | None = None) -> None:
    ...
    bindings = build_binding_map(
        stage_func_def, call_args, caller_bindings or {},
        kernel_slot_map or {}, cross_buffers)
    if not bindings:
        return
    ...
    overlap_map = _build_stage_overlap_map(
        addr_overlaps, cross_buffers, slot_to_buffer, stage_cc_buffers)
    ...
    _scan_accesses_recursive(
        stage_func_def, slot_to_buffer, cross_buffers, all_buffer_memory,
        vf_func_defs, cross_access, overlap_map, local_access,
        bindings, kernel_slot_map or {}, stage_func_defs or {})

    # Export results
    if cross_access_out is not None:
        for buf_name, (role, first_pipe, last_pipe) in cross_access.items():
            cross_access_out.append(CrossCoreAccess(
...
def _build_stage_overlap_map(addr_overlaps, cross_buffers,
                             slot_to_buffer, stage_cc_buffers) -> dict:
...
def _validate_slot_accessors(stage_func_def: ast.FunctionDef,
                             group_param_names: set[str]) -> None:
    ...
    for node in ast.walk(stage_func_def):
        ...
        if not (isinstance(node.func.value, ast.Name)
                and node.func.value.id in group_param_names):
            continue
