...
from hc_pre_impl import check_input_output_shape_dtype, hc_pre_kernel, hc_pre_kernel_prefill
import pytest
import torch
...
def test_hc_pre_inmodel(t=16):
    ...
    model = torch.compile(HcPre(), dynamic=False, fullgraph=True, backend=npu_backend)
    for _ in range(1):
