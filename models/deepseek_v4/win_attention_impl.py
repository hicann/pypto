...
def check_args_tnd(
    ...
    assert (
        cu_seqlens_q.dtype == torch.int and cu_seqlens_q.ndim == 1 and cu_seqlens_q.shape[0] == seqused_kv.shape[0] + 1
    ), (
...
@pypto.frontend.jit(
    ...
    pass_options={"cube_l1_reuse_setting": {0: 4}, "cube_nbuffer_setting": {1: 2}, "vec_nbuffer_setting": {-1: 4}},
)
    ...
    for b_idx in pypto.loop(b, name="LOOP_B", idx_name="B_idx"):
        ...
        for g_idx in pypto.loop(groups_num, name="LOOP_G", idx_name="G_idx"):
            ...
            q_tensor_cur = pypto.view(
                q_2d, [group_size * n_q, d_q], [cur_offset, 0], valid_shape=[valid_group_len * n_q, d_q]
            )
...
pyptolib.define(
    "sliding_window_attention(Tensor q, Tensor ori_block_table, Tensor ori_kv, Tensor seqused_kv, \
...
def sliding_window_attention(q, ori_block_table, ori_kv, seqused_kv, sinks, win_size, mask, cu_seqlens_q):
...
try:
    ...
    def sliding_window_attention(q, ori_block_table, ori_kv, seqused_kv, sinks, win_size, mask, cu_seqlens_q):
        return deepseekv4_win_atten(q, ori_block_table, ori_kv, seqused_kv, sinks, win_size, mask, cu_seqlens_q)
except Exception as e:
    if "could not parse dispatch key: NPU" in str(e):
        print("Skip: torchair not installed, skip NPU registration for operator 'sliding_window_attention'")
    else:
        print(f"Skip: Unexpected error : {e}")


def sliding_win_atten_graph(
    q: torch.Tensor,
    ori_block_table: torch.Tensor,
    ori_kv: torch.Tensor,
    seqused_kv: torch.Tensor,
