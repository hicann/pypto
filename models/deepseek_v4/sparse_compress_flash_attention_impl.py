...
def sparse_compress_flash_attention_compute(
    query,
    actual_seq_q,
    ori_kv,
    cmp_kv,
    ...
    for batch_idx in pypto.loop(0, batch_size_sym, 1, name="LOOP_L0_idx", idx_name="bIdx"):
        ...
        for slc_idx in pypto.loop(0, cur_s1, 1, name="LOOP_L1_s1_SA", idx_name="s1Idx"):
            ...
            kv_block_0 = pypto.view(
                ori_kv,
                [win_size, d],
                [physical_block_id_0 * block_size + start_offset, 0],
            ...
            if pypto.cond(start_block < end_block):
                ...
                kv_block_1 = pypto.view(
                    ori_kv, [win_size, d], [physical_block_id_1 * block_size, 0], valid_shape=[cur_kv_block_1_size, d]
                )
                pypto.assemble(pypto.clone(kv_block_1), [cur_kv_block_0_size, 0], kj)

            # ---- gather: GM --> UB  [topk_tile, d]
            pypto.set_vec_tile_shapes(128, 512)
            cur_cmp_sparse_indices = pypto.view(
                cmp_sparse_indices, [1, topk_tile], [t_idx, 0], valid_shape=[1, cur_topk_size]
            )
...
def sparse_compress_flash_attention_kernel(
    ...
    sparse_compress_flash_attention_compute(
        query,
        actual_seq_q,
        ori_kv,
        cmp_kv,
...
def check_input_output_shape_dtype(
    query_npu, q_act_seqs_npu, ori_kv_npu, cmp_kv_npu, atten_sink_npu, cmp_sparse_indices_npu
):
    ...
    assert cmp_sparse_indices_npu.dim() == 2, (
        f"cmp_sparse_indices_npu dim num is {cmp_sparse_indices_npu.dim()}, \
        expected 2"
    )


@allow_in_graph
def npu_sparse_compress_flash_attention(
    query_npu,
    q_act_seqs_npu,
    ori_kv_npu,
    cmp_kv_npu,
    ...
    assert not isinstance(query_npu, FakeTensor), "query_npu is FakeTensor"
    ...
    attention_out_npu = torch.zeros(
        [query_npu.size(0), query_npu.size(1)], dtype=query_npu.dtype, device=f'{query_npu.device}'
    )
    ...
    sparse_compress_flash_attention_kernel(
        *tensors, nq, n_kv, softmax_scale, topk, block_size, win_size, cmp_ratio, tile_config
    )
...
pyptolib.define(
    "sparse_compress_flash_attention(Tensor query_npu, Tensor q_act_seqs_npu, Tensor ori_kv_npu, \
...
def sparse_compress_flash_attention(
    query_npu,
    q_act_seqs_npu,
    ori_kv_npu,
    cmp_kv_npu,
    ...
    y = torch.empty(
        [
            ori_block_table_npu.size(0),
            cmp_sparse_indices_npu.size(0) // ori_block_table_npu.size(0),
            query_npu.size(0) // cmp_sparse_indices_npu.size(0),
            query_npu.size(1),
        ],
        dtype=query_npu.dtype,
...
try:
    ...
    def sparse_compress_flash_attention(
        ...
        return npu_sparse_compress_flash_attention(
            query_npu,
            q_act_seqs_npu,
            ori_kv_npu,
            cmp_kv_npu,
    ...
    if "could not parse dispatch key: NPU" in str(e):
        print("Skip: torchair not installed, skip NPU registration for operator 'sparse_compress_flash_attention'")
    else:
        print(f"Skip: Unexpected error : {e}")


def sparse_compress_flash_attention_graph(
    query_npu,
    q_act_seqs_npu,
    ori_kv_npu,
    cmp_kv_npu,
