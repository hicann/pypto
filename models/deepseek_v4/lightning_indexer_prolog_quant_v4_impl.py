...
def check_input_shape_dtype(qr, idx_wq_b, x, weights_proj, cos, sin, hadamard, qr_scale, idx_wq_b_scale):
    q_lora_rank = 1024
    idx_nq = 64
    head_dim = 128
    rope_dim = 64
    h = 4096
    assert len(qr.shape) == 2 and qr.size(1) == q_lora_rank, (
        f"qr shape need to be: (t, f{q_lora_rank}), but got: f{qr.shape}"
    )
    assert len(idx_wq_b.shape) == 2 and idx_wq_b.size(0) == q_lora_rank and idx_wq_b.size(1) == idx_nq * head_dim, (
        f"idx_wq_b shape need to be: (f{q_lora_rank, idx_nq * head_dim}), but got: f{idx_wq_b.shape}"
    )
    assert len(x.shape) == 2 and x.size(1) == h, f"x shape need to be: (t, f{h}), but got: f{x.shape}"
    assert len(weights_proj.shape) == 2 and weights_proj.size(0) == h and weights_proj.size(1) == idx_nq, (
        f"weights_proj shape need to be: (f{h, idx_nq}), but got: f{weights_proj.shape}"
    )
    assert len(cos.shape) == 2 and cos.size(1) == rope_dim, (
        f"cos shape need to be: (t, f{rope_dim}), but got: f{cos.shape}"
    )
    assert len(sin.shape) == 2 and sin.size(1) == rope_dim, (
        f"sin shape need to be: (t, f{rope_dim}), but got: f{sin.shape}"
    )
    assert len(hadamard.shape) == 2 and hadamard.size(0) == head_dim and hadamard.size(1) == head_dim, (
        f"hadamard shape need to be: (f{head_dim, head_dim}), but got: f{hadamard.shape}"
    )
    assert len(qr_scale.shape) == 2 and qr_scale.size(1) == 1, (
        f"qr_scale shape need to be: (t, f{1}), but got: f{qr_scale.shape}"
    )
    assert (
        len(idx_wq_b_scale.shape) == 2 and idx_wq_b_scale.size(0) == idx_nq * head_dim and idx_wq_b_scale.size(1) == 1
    ), f"idx_wq_b_scale shape need to be: (f{idx_nq * head_dim, 1}), but got: f{idx_wq_b_scale.shape}"

    assert qr.dtype == idx_wq_b.dtype == torch.int8, (
        f"expected qr and idx_wq_b dtype to be torch.int8, but got: f{qr.dtype} and f{idx_wq_b.dtype}"
    )
    assert x.dtype == weights_proj.dtype == cos.dtype == sin.dtype == hadamard.dtype == torch.bfloat16, (
    ...
    assert qr_scale.dtype == idx_wq_b_scale.dtype == torch.float32, (
...
def npu_quant_lightning_indexer_prolog(
    ...
    check_input_shape_dtype(qr, idx_wq_b, x, weights_proj, cos, sin, hadamard, qr_scale, idx_wq_b_scale)
...
pyptolib.define(
    "quant_lightning_indexer_prolog(Tensor qr, Tensor idx_wq_b, Tensor x, Tensor weights_proj, "
    "Tensor cos, Tensor sin, Tensor hadamard, Tensor qr_scale, Tensor idx_wq_b_scale) "
    "-> (Tensor, Tensor, Tensor)"
)


@torch.library.impl(pyptolib, "quant_lightning_indexer_prolog", "Meta")
def quant_lightning_indexer_prolog(qr, idx_wq_b, x, weights_proj, cos, sin, hadamard, qr_scale, idx_wq_b_scale):
    q = torch.empty(
        [qr.size(0), weights_proj.size(1), hadamard.size(0)],
        dtype=qr.dtype,
        device=qr.device,
...
def quant_lightning_indexer_prolog(  # noqa: F811
