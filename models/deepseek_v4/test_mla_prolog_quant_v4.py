...
def mla_prolog_compute(inputs):
    ...
    _q_a_layernorm_out = q_a_layernorm.to(torch.bfloat16)
...
def gen_mla_prolog_data(params, dtype, is_quant=True, is_nz=False):
    x, wq_a, wq_b, w_kv, gamma_cq, gamma_ckv, cos, sin, wq_b_scale = gen_mla_prolog_input_data(
        params, dtype, is_quant, is_nz
    )
    inputs = dict()
    inputs["x"] = x
    inputs["wq_a"] = wq_a
