...
def hc_split_sinkhorn(
    comb_flag: pypto.Tensor, hc_split_sinkhorn_iters, hc_eps
...
def hc_pre_kernel(
    ...
    for t_idx, unroll_length in pypto.loop_unroll(0, t, 1, name="t_loop", idx_name="t_idx", unroll_list=unroll_list):
        tile_t = unroll_length
        ...
        if tile_t <= 32:
            ...
            tile_shape_2 = 1
            pypto.set_cube_tile_shapes([16, 16], [512, 1024], [128, 128], enable_split_k=False)
        elif tile_t <= 64:
            split_k = True
            ...
            tile_shape_2 = 4
            pypto.set_cube_tile_shapes([16, 16], [512, 2 * 1024], [128, 128], enable_split_k=True)
...
def check_input_output_shape_dtype(
    x: torch.Tensor, hc_fn: torch.Tensor, hc_scale: torch.Tensor, hc_base: torch.Tensor, hc_mult: int = 4
):
    ...
    assert hc_scale.dim() == 1 and hc_scale.size(0) == 3, (
        f"hc_scale dim num {hc_scale.dim()}, \
            hc_scale axis0 is {hc_scale.size(0)}, expected 1, 3"
    )
    assert hc_base.dim() == 1 and hc_base.size(0) == mix_hc, (
        f"hc_scale dim num {hc_base.dim()}, \
...
pyptolib.define(
    "hc_pre(Tensor x, Tensor hc_fn, Tensor hc_scale, Tensor hc_base, int hc_mult, "
    "int hc_split_sinkhorn_iters, float hc_eps) -> (Tensor, Tensor, Tensor)"
)
...
try:
    ...
    if "could not parse dispatch key: NPU" in str(e):
        print("Skip: torchair not installed, skip NPU registration for operator 'hc_pre'")
    else:
...
def npu_hc_pre(
    ...
    _shapes = [x.shape, hc_fn.shape, hc_scale.shape, hc_base.shape, y.shape, post.shape, comb.shape]
