...
def attention(
    ...
    if debug_str in ["true", "1", "yes"]:
        unroll_list = [8, 4, 2, 1]
    ifa_func(*pto_inputs, *pto_outputs, unroll_list, enable_residual, eps, num_decode_tokens)
...
def ifa_func(block_table, kv_act_seqs, index, x, residual_input, x_gamma, \
             x_bias, x_scale, x_offset, weight, quant_bias, deq_scale, q_gamma, q_bias, k_gamma, \
             k_bias, cos, sin, \
             atten_out, q_tmp, k_tmp, v_tmp, residual, key_cache, value_cache, unroll_list, \
             enable_residual=True, eps=1e-05, num_decode_tokens=0):
    bs_tile = 8
...
def test_attention():
    ...
    slot_mapping = torch.randperm(block_num * block_size)[:b].to(npu)
