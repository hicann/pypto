...
struct TileScalarTypes {
    TileTypePtr tile_type;
...
/**
 * \brief Validate a TileType + ScalarType argument pair and return the narrowed types
 *
             ...
             const std::optional<T>& default_value = std::nullopt)
{
    for (const auto& kwarg : kwargs) {
        if (kwarg.first == key) {
            return AnyCast<T>(kwarg.second, "kwarg key: " + key);
        }
    }
    if (default_value) {
...
/**
 * \brief Compute the product of shape dimensions when all dimensions are static
 *
 * \param shape Shape vector to inspect
 * \return Product of constant dimensions, or -1 if any dimension is dynamic
 */
int64_t ComputeStaticShapeProduct(const std::vector<ExprPtr>& shape);
