    ...
    for (size_t i = 0; i < output.size(); ++i) {
        if (alignIdx < alignedShape.size() && alignedShape[alignIdx] == output[i]) {
            if (ChangingAxis[alignIdx] && !dynOutput[i].IsImmediate()) {
                APASS_LOG_WARN_F(Elements::Tensor, "Found undetermined axis from dynOutput[%zu], which is also a merged/split axis.", i);
                return WARNING;
            }
            alignIdx++;
            continue;
        ...
        while (curVal > 1) {
            if (alignedShape[alignIdx] == 0) {
                APASS_LOG_ERROR_F(Elements::Tensor, "Incorrect alignedShape, alignedShape[%zu] is 0; Please check whether the alignshape generated by the ShapeAlign function is corrent.", alignIdx);
                return FAILED;
            }
            curVal /= alignedShape[alignIdx++];
        }
...
Status SplitReshape::ShapeAlign(
    ...
    while (i1 < shape1.size() || i2 < shape2.size()) {
        ...
        if ((i1 >= shape1.size() || i2 >= shape2.size())) {
            APASS_LOG_ERROR_F(Elements::Tensor, "Shape1 index i1(%zu) exceeds shape1 size(%zu) or shape2 index i2(%zu) exceeds shape2 size(%zu); Please check the process of compute alignShape.", i1, shape1.size(), i2, shape2.size());
            return FAILED;
        }
...
Status SplitReshape::UpdateShapeOffset(UpdatePara &para, bool &flag, int &currentShape, int &currentOffset) {
    ...
    if (flag && currentShape <= stride) {
        ...
        if (currentOffset + currentShape > stride) {
            APASS_LOG_WARN_F(Elements::Tensor, "Found tail cond, currentOffset is %ld, currentShape is %ld, stride is %ld.", static_cast<long>(currentOffset), static_cast<long>(currentShape), static_cast<long>(stride));
            return WARNING;
        }
        return SUCCESS;
    }
    if (currentOffset % stride != 0) {
        APASS_LOG_WARN_F(Elements::Tensor, "Found tail cond, currentOffset is %ld, stride is %ld.", static_cast<long>(currentOffset), static_cast<long>(stride));
        return WARNING;
    }
    if (currentShape % stride != 0) {
        APASS_LOG_WARN_F(Elements::Tensor, "Found tail cond, currentShape is %ld, stride is %ld.", static_cast<long>(currentShape), static_cast<long>(stride));
        return WARNING;
    }
    para.OffsetVal = currentOffset / stride;
...
Status SplitReshape::ConstructShapeOffset(const ReshapeTilePara &shapePara, size_t &i, size_t j,
    ...
    while (stride > 1) {
        if (stride % alignedShape[i] != 0) {
            APASS_LOG_WARN_F(Elements::Tensor, "Cannot cut shape, stride is %ld, alignedShape[%zu] is %ld.", static_cast<long>(stride), i, static_cast<long>(alignedShape[i]));
            return WARNING;
        }
    ...
    for (size_t j = 0UL; j < shape.size(); j++) {
        if (shape[j] == 1) {
            if (tileOffset[j] != 0 || tileShape[j] != 1) {
                APASS_LOG_ERROR_F(Elements::Tensor, "Shape[%zu] is 1, but tileOffset[%zu] is %ld (should be 0) and tileShape[%zu] is %ld (should be 1); The tileshape is incorrect.", j, j, static_cast<long>(tileOffset[j]), j, static_cast<long>(tileShape[j]));
                return FAILED;
            }
            continue;
        }
...
Status SplitReshape::AlignToRaw(
    ...
    for (size_t j = 0UL; j < newRawshape.size(); j++) {
        ...
        while (stride > 1) {
            if (rawShape[i] == 0) {
                APASS_LOG_ERROR_F(Elements::Tensor, "Incorrect rawShape, rawShape[%zu] is 0; Please set the correct rawShape.", i);
                return FAILED;
            }
            if (stride % rawShape[i] != 0) {
                APASS_LOG_ERROR_F(Elements::Tensor, "Incorrect alignment, rawShape[i] %ld should divides %ld stride; Please set the correct alignment.", static_cast<long>(rawShape[i]), static_cast<long>(stride));
                return FAILED;
            }
            stride /= rawShape[i];
