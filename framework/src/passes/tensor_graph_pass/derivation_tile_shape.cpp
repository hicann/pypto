...
static bool CanNotAlignShape(const std::vector<int64_t> &inShape, const std::vector<int64_t> &outShape, int64_t inSize, int64_t outSize) {
    ...
    while (ctx.i < inShape.size() || ctx.o < outShape.size()) {
        ...
        if (ctx.i >= inShape.size()) {
            ...
            while (ctx.o < outShape.size() && outShape[ctx.o] == 1) {
                ProcessSplitAndKeep(ctx.a, ctx.o, outShape[ctx.o], inStatus[inShape.size()-1], alignedShape, alignedStatus);
                ctx.o++;
            }
            continue;
        }
        if (ctx.o >= outShape.size()) {
            ...
            while (ctx.i < inShape.size() && inShape[ctx.i] == 1) {
                ProcessKeepAndKeep(ctx.a, (outShape.size() -1), inShape[ctx.i], inStatus[ctx.i], alignedShape, alignedStatus);
                ctx.i++;
            }
            continue;
        }
...
static Status HandleSplitTileShape(std::vector<size_t> alignedIndexes,
    int64_t &tileShape, std::vector<ShapeStatus> &alignedStatus) {
    ...
    std::vector<std::vector<int64_t>> &allActTileShape, std::vector<std::vector<int64_t>> &allActOffset)
{
    if (static_cast<size_t>(dimIdx) == inShape.size()) {
        tileCnt++;
        APASS_LOG_DEBUG_F(Elements::Tensor, "tileCnt:%ld, actOffset%s, actTileShape%s",
            tileCnt, GetStr(actOffset).c_str(), GetStr(actTileShape).c_str());
...
static int64_t calcTileSubDistance(int64_t currSub, const std::vector<int64_t> &currTile, const Stride currStride,
    ...
    /* 计算第currSub个元素，在tile块上的坐标，例如对于5*4*3为shape的张量，currSub=0对应[0, 0, 1], currSub=12对应[1, 0, 0] */
    ...
    APASS_LOG_DEBUG_F(Elements::Tensor, "currSub:%ld, currTileSub%s in tile",
        currSub, GetStr(currTileSub).c_str());
    /* 将tile块上的坐标，转移到整个shape上的坐标，进行坐标的偏移 */
    for (k = 0; k < currTile.size(); k++) {
        currTileSub[k] = currTileSub[k] + actOffset[k];
    }
    APASS_LOG_DEBUG_F(Elements::Tensor, "currSub:%ld, currTileSub%s in tensor",
        currSub, GetStr(currTileSub).c_str());
...
static std::vector<int64_t> GetTileCntShape(const std::vector<int64_t> &shape, const std::vector<int64_t> &tileShape) {
    ...
    APASS_LOG_DEBUG_F(Elements::Tensor, "shape:%s, tileShape:%s, tileCntShape:%s",
        GetStr(shape).c_str(), GetStr(tileShape).c_str(), GetStr(tileCntShape).c_str());
    return tileCntShape;
}
...
static Status CheckTileShape(const std::vector<int64_t> &inShape, const std::vector<int64_t> &outShape,
    ...
    if (inTileCnt != ouTileCnt || inTileCnt <= 0 || ouTileCnt <= 0) {
        /* 检查tile展开后的个数是否相等 */
        APASS_LOG_WARN_F(Elements::Operation, "inTileCnt:%ld is not same as ouTileCnt:%ld",
            inTileCnt, ouTileCnt);
        return WARNING;
    }
    ...
    APASS_LOG_DEBUG_F(Elements::Operation, "inStride:%s, outStride:%s, inTileCntShape:%s, outTileCntShape:%s",
        GetStr(inStride).c_str(), GetStr(outStride).c_str(), GetStr(inTileCntShape).c_str(), GetStr(outTileCntShape).c_str());
    ...
    for (int64_t i = 0; i < inTileCnt; ++i) {
        ...
        if (inTileSize != outTileSize) {
            APASS_LOG_WARN_F(Elements::Tensor, "tileCnt:%ld, inTileSize:%ld is not equal to outTileSize:%ld",
        ...
        for (int64_t j = 0; j < inTileSize; j++) {
            ...
            if (inDistance != outDistance) {
                APASS_LOG_WARN_F(Elements::Tensor, "Cnt:%ld %ld, inDistance:%ld is not same as outDistance:%ld",
