...
{
    if (auto e = As<Lt>(cond))
        return std::make_shared<Ge>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Le>(cond))
        return std::make_shared<Gt>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Gt>(cond))
        return std::make_shared<Le>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Ge>(cond))
        return std::make_shared<Lt>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Eq>(cond))
        return std::make_shared<Ne>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Ne>(cond))
        return std::make_shared<Eq>(e->left_, e->right_, DataType::BOOL, span);
    if (auto e = As<Not>(cond))
        return e->operand_; // NOT (NOT x) → x
    ...
    const Span& span)
{
    ...
    iter_args.insert(iter_args.end(), original_iter_args.begin(), original_iter_args.end());

    std::vector<VarPtr> return_vars = {return_var};
    return_vars.insert(return_vars.end(), original_return_vars.begin(), original_return_vars.end());

    return {flag_type, iter_arg, std::move(iter_args), std::move(return_vars)};
}
...
std::optional<StmtPtr> MakeIterArgYieldStmt(const std::vector<IterArgPtr>& iter_args, const Span& span)
{
    if (iter_args.empty())
        return std::nullopt;
    return std::make_shared<YieldStmt>(MakeIterArgYieldValues(iter_args), span);
}
...
StmtPtr LowerContinueInStmt(const StmtPtr& stmt, const std::vector<IterArgPtr>& iter_args)
{
    ...
    if (auto if_stmt = As<IfStmt>(stmt)) {
        ...
        StmtPtr new_then = LowerContinueInStmt(if_stmt->thenBody_, iter_args);
    ...
    if (!lowered_remaining.empty()) {
        ...
        result.push_back(
            std::make_shared<IfStmt>(
                if_stmt->condition_, std::move(then_body), std::make_optional(std::move(else_body)),
    ...
    const IfStmtPtr& if_stmt, std::vector<StmtPtr> lowered_remaining, const std::vector<IterArgPtr>& iter_args)
{
    ...
    result.push_back(
        std::make_shared<IfStmt>(
            std::move(negated_cond), std::move(guard_body), std::move(else_body), if_stmt->returnVars_,
            if_stmt->span_));
    return result;
}
    ...
    const std::vector<IterArgPtr>& iter_args)
{
    ...
    std::vector<StmtPtr> pre_stmts(then_stmts.begin(), then_stmts.end() - 1);
    std::vector<StmtPtr> remaining(stmts.begin() + stmt_index + 1, stmts.end());
    auto lowered_remaining = LowerContinueInStmtList(remaining, iter_args);

    if (!pre_stmts.empty()) {
        return BuildThenContinueWithPreStmts(if_stmt, std::move(pre_stmts), std::move(lowered_remaining), iter_args);
    }
    ...
    const std::vector<IterArgPtr>& iter_args)
{
    ...
    std::vector<StmtPtr> remaining(stmts.begin() + stmt_index + 1, stmts.end());
    auto lowered_remaining = LowerContinueInStmtList(remaining, iter_args);
    auto lowered_body = LowerContinueInStmtList(FlattenToStmtList(if_stmt->thenBody_), iter_args);
...
std::vector<StmtPtr> LowerBareContinue(const StmtPtr& stmt, const std::vector<IterArgPtr>& iter_args)
{
    std::vector<StmtPtr> result;
    if (!iter_args.empty()) {
        result.push_back(std::make_shared<YieldStmt>(MakeIterArgYieldValues(iter_args), stmt->span_));
    }
    return result;
}
    ...
    const std::vector<StmtPtr>& stmts, const std::vector<IterArgPtr>& iter_args)
{
    ...
    for (size_t i = 0; i < stmts.size(); ++i) {
        ...
        if (auto if_stmt = As<IfStmt>(stmt)) {
            if (auto lowered = LowerThenContinuePattern(if_stmt, stmts, i, iter_args)) {
                return *lowered; // All remaining statements have been absorbed
            }

            if (auto lowered = LowerElseContinuePattern(if_stmt, stmts, i, iter_args)) {
                return *lowered;
            }
        }

        if (As<ContinueStmt>(stmt) != nullptr) {
            return LowerBareContinue(stmt, iter_args); // Drop remaining statements
        }
    ...
    const std::vector<StmtPtr>& stmts, const std::vector<IterArgPtr>& iter_args, int& var_counter, const Span& span)
{
    for (size_t i = 0; i < stmts.size(); ++i) {
        ...
        for (size_t j = 0; j < iter_args.size(); ++j) {
            auto r_ia = std::make_shared<Var>(
                "_cont_ia" + std::to_string(var_counter++) + "_" + std::to_string(j), iter_args[j]->GetType(), span);
    ...
    if (!stmts.empty()) {
        if (auto trailing = As<YieldStmt>(stmts.back())) {
            ...
            for (const auto& v : trailing->value_)
                no_break_exprs.push_back(v);
        } else {
...
ForStmtPtr LowerBreakInFor(const ForStmtPtr& op)
{
    Span span = op->span_;
    int var_counter = 0;
    auto break_control = MakeBreakControlState(op->iterArgs_, op->returnVars_, span);
    ...
    auto [lowered, result_exprs] = LowerBreakToValue(body_stmts, op->iterArgs_, var_counter, span);
    ...
    std::vector<ExprPtr> skip_yield = {break_control.iter_arg};
    for (const auto& ia : op->iterArgs_)
        skip_yield.push_back(ia);
    ...
    for (size_t i = 0; i < op->iterArgs_.size(); ++i) {
        auto g = std::make_shared<Var>(
            "_guard_ia" + std::to_string(var_counter++) + "_" + std::to_string(i), op->iterArgs_[i]->GetType(), span);
    ...
    auto guard_if = std::make_shared<IfStmt>(
        break_control.iter_arg, std::move(then_body), std::optional<StmtPtr>(std::move(else_body)),
...
WhileStmtPtr LowerBreakInWhile(const WhileStmtPtr& op)
{
    ...
    ExprPtr new_condition = break_control.iter_arg;
    ...
    for (size_t i = 0; i < op->iterArgs_.size(); ++i) {
        auto ov = std::make_shared<Var>(
            "_outer_ia" + std::to_string(var_counter++) + "_" + std::to_string(i), op->iterArgs_[i]->GetType(), span);
