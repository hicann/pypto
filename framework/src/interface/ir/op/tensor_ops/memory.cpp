                               ...
                               [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    PRO_IR_CHECK(ExternalError::INVALID_ARGUMENT, args.size() == 1)
        << "tensor.create requires exactly 1 argument (shape tuple), but got " << args.size();
    ...
    PRO_IR_CHECK(ExternalError::INVALID_ARGUMENT, found_dtype) << "tensor.create requires 'dtype' kwarg";
    ...
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, shape_tuple_type)
        << "tensor.create requires shape to be TupleType, but got " << args[0]->GetType()->TypeName();
    ...
    for (size_t i = 0; i < shape_tuple_type->types_.size(); ++i) {
        ...
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type)
            << "tensor.create shape tuple element " << i << " must be ScalarType, but got "
    ...
    // Otherwise use GetItemExpr for runtime tuples
                             ...
                             [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    PRO_IR_CHECK(ExternalError::INVALID_ARGUMENT, args.size() == 3)
        << "tensor.view requires exactly 3 arguments (input, shape, offset), but got " << args.size();

    // First argument must be TensorType
    auto tensor_type = As<TensorType>(args[0]->GetType());
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, tensor_type)
        << "tensor.view requires first argument to be a TensorType, but got " << args[0]->GetType()->TypeName();

    // Second argument must be TupleType (shape)
    auto shape_tuple_type = As<TupleType>(args[1]->GetType());
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, shape_tuple_type)
        << "tensor.view requires shape to be TupleType, but got " << args[1]->GetType()->TypeName();

    // Validate all shape elements are ScalarType with integer dtype
    for (size_t i = 0; i < shape_tuple_type->types_.size(); ++i) {
        auto scalar_type = As<ScalarType>(shape_tuple_type->types_[i]);
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type)
            << "tensor.view shape tuple element " << i << " must be ScalarType, but got "
            << shape_tuple_type->types_[i]->TypeName();
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type->dtype_.IsInt())
    ...
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, offset_tuple_type)
        << "tensor.view requires offset to be TupleType, but got " << args[2]->GetType()->TypeName();
    ...
    for (size_t i = 0; i < offset_tuple_type->types_.size(); ++i) {
        ...
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type)
            << "tensor.view offset tuple element " << i << " must be ScalarType, but got "
                                 ...
                                 [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, target_type)
        << "tensor.assemble requires first argument to be a TensorType, but got " << args[0]->GetType()->TypeName();

    // Second argument (source) must be TensorType
    auto source_type = As<TensorType>(args[1]->GetType());
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, source_type)
        << "tensor.assemble requires second argument to be a TensorType, but got " << args[1]->GetType()->TypeName();

    // Third argument must be TupleType (offset)
    auto offset_tuple_type = As<TupleType>(args[2]->GetType());
    PRO_IR_CHECK(ExternalError::INVALID_TYPE, offset_tuple_type)
        << "tensor.assemble requires offset to be TupleType, but got " << args[2]->GetType()->TypeName();

    // Validate all offset elements are ScalarType with integer dtype
    for (size_t i = 0; i < offset_tuple_type->types_.size(); ++i) {
        auto scalar_type = As<ScalarType>(offset_tuple_type->types_[i]);
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type)
            << "tensor.assemble offset tuple element " << i << " must be ScalarType, but got "
            << offset_tuple_type->types_[i]->TypeName();
        PRO_IR_CHECK(ExternalError::INVALID_TYPE, scalar_type->dtype_.IsInt())
