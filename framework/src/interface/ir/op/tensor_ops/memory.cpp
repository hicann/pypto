    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    CHECK(args.size() == 1) << "tensor.create requires exactly 1 argument (shape tuple), but got " << args.size();
    ...
    for (size_t i = 0; i < shape_tuple_type->types_.size(); ++i) {
        ...
        CHECK(scalar_type->dtype_.IsInt()) << "tensor.create shape tuple element " << i
    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    CHECK(shape_tuple_type) << "tensor.view requires shape to be TupleType, but got " << args[1]->GetType()->TypeName();
    ...
    for (size_t i = 0; i < shape_tuple_type->types_.size(); ++i) {
        ...
        CHECK(scalar_type) << "tensor.view shape tuple element " << i << " must be ScalarType, but got "
                           << shape_tuple_type->types_[i]->TypeName();
        CHECK(scalar_type->dtype_.IsInt()) << "tensor.view shape tuple element " << i
                                           << " must have integer dtype, but got " << scalar_type->dtype_.ToString();
    }
    ...
    for (size_t i = 0; i < offset_tuple_type->types_.size(); ++i) {
        ...
        CHECK(scalar_type->dtype_.IsInt()) << "tensor.view offset tuple element " << i
                                           << " must have integer dtype, but got " << scalar_type->dtype_.ToString();
    }
    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    for (size_t i = 0; i < offset_tuple_type->types_.size(); ++i) {
        ...
        CHECK(scalar_type) << "tensor.assemble offset tuple element " << i << " must be ScalarType, but got "
                           << offset_tuple_type->types_[i]->TypeName();
        CHECK(scalar_type->dtype_.IsInt()) << "tensor.assemble offset tuple element " << i
                                           << " must have integer dtype, but got " << scalar_type->dtype_.ToString();
    }
    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    if (axis < 0)
        axis += rank;
    CHECK(axis >= 0 && axis < rank) << "tensor.dim axis " << axis_const->value_ << " out of range for tensor of rank "
                                    << rank;
...
TypePtr DeduceTensorGetValType(
    [[maybe_unused]] const std::vector<ExprPtr>& args,
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    CHECK(offset_type) << "tensor.getval requires offset to be ScalarType, but got " << args[1]->GetType()->TypeName();
    CHECK(offset_type->dtype_.IsInt()) << "tensor.getval offset must have integer dtype, but got "
...
TypePtr DeduceTensorSetValType(
    [[maybe_unused]] const std::vector<ExprPtr>& args,
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    // Second argument must be ScalarType with integer dtype (linear offset)
    auto offset_type = As<ScalarType>(args[1]->GetType());
    CHECK(offset_type) << "tensor.setval requires offset to be ScalarType, but got " << args[1]->GetType()->TypeName();
    CHECK(offset_type->dtype_.IsInt()) << "tensor.setval offset must have integer dtype, but got "
                                       << offset_type->dtype_.ToString();

    // Third argument must be ScalarType (value to write)
    auto value_type = As<ScalarType>(args[2]->GetType());
    CHECK(value_type) << "tensor.setval requires value to be ScalarType, but got " << args[2]->GetType()->TypeName();
