...
REGISTER_OP("struct.create")
    ...
    .set_description(
        "Materialize a named tuple into a C++ struct. args are field values; kwargs `name` "
...
REGISTER_OP("struct.set")
    .set_op_category("StructOp")
    .set_description(
        "Statement side-effect call: write one struct field. args are (base, value); kwarg `field` "
    ...
    .f_deduce_type(
        [](const std::vector<ExprPtr>& args,
           const std::vector<std::pair<std::string, std::any>>& kwargs) -> TypePtr {
            ...
            CHECK(args[0]->GetType()->GetKind() == ObjectKind::TupleType)
