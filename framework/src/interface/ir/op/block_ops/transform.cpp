...
int NormalizeAxis(int axis, size_t ndim)
{
    ...
    CHECK(axis >= 0 && axis < static_cast<int>(ndim)) << "Axis " << axis << " is out of range for " << ndim << "D tile";
    return axis;
}
 ...
 */
void ValidateIndexTupleElements(const TupleTypePtr& tuple_type, const std::string& op_name, const std::string& arg_name)
{
    for (size_t i = 0; i < tuple_type->types_.size(); ++i) {
        auto scalar_type = As<ScalarType>(tuple_type->types_[i]);
        CHECK(scalar_type) << op_name << " " << arg_name << " tuple element " << i << " must be ScalarType, but got "
    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs)
{
    ...
    CHECK(shape_tuple_type) << "tile.view requires shape to be TupleType, but got " << args[1]->GetType()->TypeName();
    ...
    CHECK(offset_tuple_type) << "tile.view requires offset to be TupleType, but got " << args[2]->GetType()->TypeName();
