...
TypePtr DeduceBlockReductionType(
    ...
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs, const std::string& op_name)
{
    ...
    int axis_value = GetOpKwarg<int>(kwargs, "axis");
    if (axis_value < 0) {
    ...
    CHECK(axis_value >= 0 && static_cast<int64_t>(axis_value) < input_ndim)
        << "The operator " << op_name << " axis " << axis_value << " is out of range for shape with " << input_ndim
        << " dimensions";
...
TypePtr DeduceBlockRowReductionType(
    [[maybe_unused]] const std::vector<ExprPtr>& args,
    [[maybe_unused]] const std::vector<std::pair<std::string, std::any>>& kwargs, const std::string& op_name)
{
    ...
    CHECK(input_ndim >= 2) << "The operator " << op_name << " requires at least a 2D tile, but got " << input_ndim
                           << " dimensions";
    ...
    return std::make_shared<TileType>(output_shape, tile_type->dtype_, std::nullopt, std::nullopt, hw_info);
}
