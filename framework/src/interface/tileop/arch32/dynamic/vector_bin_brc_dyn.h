...
/*!
 * \file vector_bin_brc_dyn.h
 * \brief
 */

// dim2 & dim1 (T0 = 1 for dim1)
template <typename T, unsigned DS, unsigned SS0, unsigned SS1, bool isInputForceCombineAxis>
                      ...
                      unsigned T2)
{
    ...
    for (int i = 0; i < T0; i++) {
        T_BIN_BRC<T, DS1, S0S1, S1S1, isInputForceCombineAxis>(dst, src0, src1, tmp, T1, T2);
        dst += DS0 * DS1;
        src0 += S0S0 * S0S1;
        src1 += S1S0 * S1S1;
    }
                      ...
                      unsigned T2, unsigned T3)
{
    ...
    for (int i = 0; i < T0; i++) {
        ...
        __ubuf__ T* tmp_ = tmp;
        for (int j = 0; j < T1; j++) {
            T_BIN_BRC<T, DS2, S0S2, S1S2, isInputForceCombineAxis>(dst_, src0_, src1_, tmp_, T2, T3);
            dst_ += DS1 * DS2;
            src0_ += S0S1 * S0S2;
            src1_ += S1S1 * S1S2;
        }
