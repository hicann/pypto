...
template <typename T>
struct RSB8ToB4Tile {
    using U = typename pto::Tile<T::Loc, typename B8ToB4<typename T::DType>::U, T::Rows, T::Cols * 2, T::BFractal, -1,
                                 -1>;
};
...
{
    typename RSB8ToB4Tile<DST>::U dst32(dst.GetValidRow(), dst.GetValidCol() * 2);
    typename RSB8ToB4Tile<SRC>::U src32(src.GetValidRow(), src.GetValidCol() * 2);
...
{
    if constexpr (where < 2) {
        return pto::MaskPattern::P1000;
    } else if constexpr (where < 4) {
        return pto::MaskPattern::P0100;
    } else if constexpr (where < 6) {
...
TILEOP void RadixSelect16BitGetB8(DST dst, SRC src)
{
    ...
    if constexpr (where % 2 == 0) {
        pto::TSHRS(dst, dst, 8);
    } else {
        pto::TANDS(dst, dst, 0xFF);
    }
}
...
TILEOP void RadixSelectHistogramPrepareB8(DST16U dst16U, TMP16U tmp16U, SRC16UF src16UF, MASK mask, USELESS useless)
{
    if constexpr (bit == 0) {
        ...
        pto::TSHLS(dst16U, dst16U, 8);
        RadixSelect16BitGetB8<bit / 8>(tmp16U, src16UF);
                                    ...
                                    TCI tci, USELESS useless)
{
    ...
    pto::TSELS(hisTmp32U, cmp, hisTmp32U, useless, 0x7fffffffu);
    pto::TEXPANDS(high, 0);
...
TILEOP void RadixSelectUpdateKTHValueB8(KTH kth, SRC src, TCI tci, TMP tmp)
{
    ...
    constexpr size_t realBit = bit % 32;
    if constexpr (realBit == 24) {
        pto::TROWEXPAND(tmp, src);
    } else {
        pto::TSHLS(tmp, src, 24 - realBit);
...
TILEOP void RadixSelectGatherB8(SELECT select, SRC src, K kth, TMP1 tmp1, TMP2 tmp2, COUNT count, CMP cmp,
                                USELESS useless)
{
    constexpr uint16_t NUM = 0xFF;
    pto::TEXPANDS(tmp1, NUM);
    pto::TEXPANDS(tmp2, NUM);
    ...
    typename RSExchangeTile<CMP>::U cmpOneRow(1, elementPerLoop / 8);
    ...
    for (LoopVar i = 0; i < validRow; ++i) {
        for (LoopVar j = 0; j < loop; ++j) {
            ...
            pto::TSELS(tmp1OneRow, cmpOneRow, tmp1OneRow, useless, ~NUM);
        }
    }
...
{
    RadixSelect16BitGetB8<(62 - bit) / 8>(tmp2U16, src);
    pto::TSHRS(tmp2U16, tmp2U16, bit % 8);
    pto::TANDS(tmp2U16, tmp2U16, 0x3);
    pto::TEXPANDS(tmp32F, 0x7FFF7FFF);
                                    ...
                                    NUM num2, NUM num3)
{
    if constexpr (bit < lastBit) {
        ...
        RadixSelectSortTwoBitB8<bit + 2, lastBit>(tmp16UF, tmpk, tmp32F, tmp16, tmp16K, select1, src16UF, srck, src32F,
                                                  src16, src16K, tmp2U16, tmp2U16K, useless, index, select2, select3,
                                                  count1, count2, count3, num0, num1, num2, num3);
    }
}
