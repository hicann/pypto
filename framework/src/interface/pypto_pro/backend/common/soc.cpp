...
#include "backend/common/cann_runtime.h"
...
{
    return std::tie(mem_type_, mem_size_, alignment_) < std::tie(other.mem_type_, other.mem_size_, other.alignment_);
}
...
bool Core::operator==(const Core& other) const { return core_type_ == other.core_type_ && mems_ == other.mems_; }
...
const SoC& CreateSoC()
{
    ...
    static SoC soc = []() {
        ...
        auto query_u64 = [&](const std::string& col, const std::string& key, uint64_t fallback) -> uint64_t {
            ...
            IR_LOGW() << "SoC spec query failed for [" << col << "]." << key << ", using default: " << fallback;
            return fallback;
        };
        ...
        mem_graph[ir::MemorySpace::Mat] = {ir::MemorySpace::Left, ir::MemorySpace::Right, ir::MemorySpace::Scaling};
