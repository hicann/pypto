...
static std::string EmitVFDuplicate(const ir::CallPtr& op, codegen::CodegenBase& codegen_base)
{
    ...
    if (is_vector_src) {
        ...
        PRO_CODEGEN_CHECK(ExternalError::INVALID_TYPE, dst_dt == src_dt)
            << "vf.full (vector-source) requires dst and src to have the same type, got dst=" << DTypeStr(dst_dt)
            << " src=" << DTypeStr(src_dt);
...
static std::string EmitVFSelect(const ir::CallPtr& op, codegen::CodegenBase& codegen_base)
{
    ...
    DataType dst_dt = GetExprDtype(op->args_[0]);
    PRO_CODEGEN_CHECK(ExternalError::INVALID_TYPE, dst_dt == DataType::INT8 || dst_dt == DataType::UINT8 ||
                                                       dst_dt == DataType::BOOL || dst_dt == DataType::INT16 ||
                                                       dst_dt == DataType::UINT16 || dst_dt == DataType::FP16 ||
                                                       dst_dt == DataType::BF16 || dst_dt == DataType::INT32 ||
                                                       dst_dt == DataType::UINT32 || dst_dt == DataType::FP32)
