...
void CompressAttentionWithTopK(const Tensor& qNope, const Tensor& qRope, const Tensor& cmpKvCache,
                               const Tensor& cmpKrCache, const Tensor& cmpBlockTable, const Tensor& actSeq,
                               const Tensor& auxTensor, Tensor& cmpAttnOut, Tensor& topkRes, const int blockSize,
                               const int cmpBlockSize, const int cmpStride, const int slcBlockSize,
                               const float softmaxScale, const int n1, const int topk, const int front, const int near,
        ...
        LOOP("CMP_ATTN_LOOP_S1", FunctionType::DYNAMIC_LOOP, s1Idx, LoopRange(s1), {}, true)
        {
            ...
            LOOP("CMP_ATTN_LOOP_BLOCK", FunctionType::DYNAMIC_LOOP, blockIdx, LoopRange(curCmpBlock), {})
            {
                ...
                LOOP("AVOID_LOOP_1", FunctionType::DYNAMIC_LOOP, avoid_loop_1_idx, LoopRange(1), {})
                {
                    ...
                    auto curCmpKv = View(cmpKvCache2D, {blockSize, dN}, {curValidSeq, dN},
                                         {curBlockIdx * blockSize, 0});
                    auto cmpKrCache2D = Reshape(
                        cmpKrCache,
                        {cmpKrCache.GetShape()[0] * cmpKrCache.GetShape()[1] * cmpKrCache.GetShape()[2], dR});
                    auto curCmpKr = View(cmpKrCache2D, {blockSize, dR}, {curValidSeq, dR},
                                         {curBlockIdx * blockSize, 0});
                ...
                LOOP("CMP_ATTN_P_SLC_FIRST", FunctionType::DYNAMIC_LOOP, slcIdx, LoopRange(curSlcLoop), {})
                {
                    ...
                    auto lastView = View(tildaPijPad, {slcWindow, n1}, {slcValid, n1},
                                         {slcIdx * slcSize, 0}); // last window
                ...
                LOOP("GEN_SLC_BEFORE_G_REDUCE", FunctionType::DYNAMIC_LOOP, unusedIdx, LoopRange(1), {})
                {
                    ...
                    IF(blockIdx != 0)
                    {
                        ...
                        auto modifyTensor = View(tildaPijPad, {cmpSize - 1, n1},
                                                 {std::min(cmpSize - 1, curValidSeq), n1}, {0, 0});
                        auto lastAuxTensor = View(auxTensor, {cmpSize - 1, n1},
                                                  {std::min(cmpSize - 1, curValidSeq), n1},
            ...
            LOOP("SOFTMAX_BLOCK", FunctionType::DYNAMIC_LOOP, blockIdx, LoopRange(curCmpBlock), {})
            {
                ...
                ELSE
                {
                    ...
                    slcBeforeGReduceBlock = Mul(slcBeforeGReduceBlock,
                                                expTmp); // (blockSlcNum, n1), (1, n1) -> (blockSlcNum, n1)
                    slcBeforeGReduceBlock = Div(slcBeforeGReduceBlock,
                                                liUpdate); // (blockSlcNum, n1), (1, n1) -> (blockSlcNum, n1)
            ...
            LOOP("TOPK_INDICES", FunctionType::DYNAMIC_LOOP, unusedIdx, LoopRange(1), {})
            {
                ...
                {
                    (void)ubReshapeIdx;
                    auto slcBeforeGReduceActual = View(slcBeforeGReduce2, {maxCmpBlock * blockSlcNum, n1},
                                                       {slcLoop, n1}, {0, 0});
                ...
                LOOP("AVOID_LOOP_6", FunctionType::DYNAMIC_LOOP, ubReshapeIdx, LoopRange(1), {})
                {
                    ...
                    ELSE
                    {
                        ...
                        auto innerTopk = std::get<1>(
                            TopK(slcReView, topk - front - near, -1, true)); // (1, 1, topk-front-near)
