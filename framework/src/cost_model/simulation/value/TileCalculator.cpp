...
namespace CostModel {
    ...
    static uint64_t LoadTile(TileState::TileStateKeyTy &k,
    ...
    static void StoreTile(TileState::TileStateKeyTy &k,
                             uint64_t &value,
                             std::shared_ptr<TileState> local,
                             std::shared_ptr<TileState> global)
    {
    ...
    inline bool IsCopyIn(const std::string &op){
        return op.find("COPY_IN") != std::string::npos;
    }
    ...
    void TileCalculator::Calculate(TileOpPtr op, FunctionInvokeInfo &invoke,
        ...
        if (op->opcode == "RESHAPE") {
            ...
            auto dk = TileState::TileKey(bind->rawMagic, bind->bufType,
