...
static uint64_t MinU64(uint64_t lhs, uint64_t rhs)
{
    return lhs < rhs ? lhs : rhs;
}

static uint64_t MaxU64(uint64_t lhs, uint64_t rhs)
{
    return lhs > rhs ? lhs : rhs;
}
}
...
static bool CanDoubleStitchDepthK(uint64_t k)
{
    return k <= static_cast<uint64_t>(UINT32_MAX) / 2;
}
...
{
    const uint32_t kForPool =
        std::min(tensorStitchDepthK, static_cast<uint32_t>(MAX_STITCH_FUNC_NUM));
...
static void BuildTensorWorkspaceFromDescriptor(WorkspaceDesc& desc, uint32_t stitchDepthK)
{
    ...
    for (const auto& root : desc.rootFuncDescList) {
        ...
        maxExclusive =
            std::max(maxExclusive, RootFunctionBudgetAtStitchDepth(root.rootTotalExclusiveOutcastRawMem, stitchDepthK));
    }
...
static uint64_t WorkspaceTotalFromDesc(
...
static uint64_t TensorWorkspaceBytesAtStitchDepth(
    const WorkspaceDesc& desc, uint32_t stitchDepthK, uint32_t parallelism, uint64_t aicoreSpilled,
    uint64_t debugTotal)
{
...
static void ComputeTensorBudgetLinearCoeffs(
    const WorkspaceDesc& desc, uint64_t& c0, uint64_t& c1)
{
    ...
    for (const auto& root : desc.rootFuncDescList) {
        ...
        maxAlignedExclusive =
            MaxU64(maxAlignedExclusive, AlignUp(root.rootTotalExclusiveOutcastRawMem, TENSOR_ADDR_ALIGNMENT));
    }
...
static uint32_t EstimateMaxStitchDepthKUpperBound(
    const WorkspaceDesc& desc, uint32_t parallelism, uint64_t aicoreSpilled, uint64_t debugTotal, uint64_t cap)
{
    if (parallelism == 0) {
        return 1;
    }
...
static uint32_t DeriveEffectiveStitchNum(
...
StitchDepthConfig ResolveStitchDepthConfig(WorkspaceDesc& desc, const RuntimeWorkspaceConfig& runtimeCfg)
{
    ...
    if (memoryDriven) {
        const uint32_t derivedK = DeriveEffectiveStitchNum(
            runtimeCfg.maxWorkspaceBytes, desc, runtimeCfg.parallelism, runtimeCfg.aicoreSpilled,
        ...
        config.encodedWorkspaceSize =
            WorkspaceTotalFromDesc(scratch, runtimeCfg.parallelism, runtimeCfg.aicoreSpilled, runtimeCfg.debugTotal);
