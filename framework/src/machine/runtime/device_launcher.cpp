...
int DeviceLauncher::LaunchAicpuKernel(rtAicpuArgsEx_t &rtArgs, bool tripleStream,
    ...
    if (tripleStream) {
        ...
        ret = rtAicpuKernelLaunchExWithArgs(rtKernelType_t::KERNEL_TYPE_AICPU_KFC, "AST_DYN_AICPU", 1, &rtArgs,
            nullptr, ctrlStream, RT_KERNEL_USE_SPECIAL_TIMEOUT);
        devRunner.ReportHostProfInfo(startTime, 1, MSPROF_GE_TASK_TYPE_AI_CPU, false);
        if (ret != RT_ERROR_NONE) {
            return ret;
        }
