...
class TestCodegenDynErf : public CodegenTestBase {
public:
    TestCodegenDynErf() : CodegenTestBase({.compileStage = CS_EXECUTE_GRAPH, .setIdGen = true}) {}
};

TEST_F(TestCodegenDynErf, ErfLayout)
{
    std::vector<int64_t> shape = {16, 16};
    TileShape::Current().SetVecTile({16, 16});
    Tensor input_a(DataType::DT_FP32, shape, "A");
    Tensor output(DataType::DT_FP32, shape, "C");
    std::string funcName = "Erf";
