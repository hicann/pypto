...
TEST_F(PyptoLaunchTest, CubeOnlyClampsWithoutQueryingVector)
{
    // No vector response is supplied: even a successful extra query is a failure.
    for (uint32_t request : {1U, 5U, 20U}) {
        SCOPED_TRACE(request);
        ExpectQuery(ACL_RT_DEV_RES_CUBE_CORE, 5);
        EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 0>(request, stream_)), request == 1 ? 1 : 5);
    }
}

TEST_F(PyptoLaunchTest, VectorOnlyClampsWithoutQueryingCube)
{
    for (uint32_t request : {1U, 7U, 20U}) {
        SCOPED_TRACE(request);
        ExpectQuery(ACL_RT_DEV_RES_VECTOR_CORE, 7);
        EXPECT_EQ((pypto::ResolveLaunchBlockDim<0, 1>(request, stream_)), request == 1 ? 1 : 7);
    }
}

TEST_F(PyptoLaunchTest, MixedBlocksRespectCubeBottleneck)
{
    ExpectMixedLimits(4, 20);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(20, stream_)), 4);
}

TEST_F(PyptoLaunchTest, MixedBlocksRoundDownOddVectorBudget)
{
...
TEST_F(PyptoLaunchTest, MixedBlocksAcceptExactlyOneCompletePair)
{
    ExpectMixedLimits(1, 2);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(20, stream_)), 1);
}
...
TEST_F(PyptoLaunchTest, TopologyUsesConfiguredCoreCounts)
{
    ...
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 1>(12, stream_)), 5);
    ExpectMixedLimits(5, 20);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<2, 3>(12, stream_)), 2);
    ExpectMixedLimits(20, 5);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<2, 3>(12, stream_)), 1);
}
...
TEST_F(PyptoLaunchTest, Uint32MaximumRemainsAPositiveBlockCount)
{
    ...
    ExpectQuery(ACL_RT_DEV_RES_VECTOR_CORE, 72);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<0, 1>(maxBlocks, stream_)), 72);
    ExpectMixedLimits(maxBlocks, maxBlocks);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(maxBlocks, stream_)), 2147483647LL);
}
...
TEST_F(PyptoLaunchTest, EveryCallObservesCurrentStreamAndLimitsAfterFailure)
{
    ...
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), 3);
    ExpectMixedLimits(8, 2);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), 1);
    uint8_t otherStreamToken = 0;
    stream_ = &otherStreamToken;
    ExpectMixedLimits(8, 1);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), -0x200000001LL);
    ExpectMixedLimits(8, 8);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), 4);
    ExpectQuery(ACL_RT_DEV_RES_CUBE_CORE, 0, 507000);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), -(0x300000000LL + 507000));
    // ACL also accepts the default stream; it must be forwarded without substitution.
    stream_ = nullptr;
    ExpectMixedLimits(2, 8);
    EXPECT_EQ((pypto::ResolveLaunchBlockDim<1, 2>(12, stream_)), 2);
}
