...
/*!
 * \file test_scatter__tensor_operation.cpp
 * \brief
 */
...
namespace Scatter_TensorOperation {
struct Scatter_TensorOpFuncArgs : public OpFuncArgs {
    Scatter_TensorOpFuncArgs(const std::vector<int64_t> &viewShape, const std::vector<int64_t> tileShape, int axis,
        ScatterMode reduce) :
...
struct Scatter_TensorOpMetaData {
    explicit Scatter_TensorOpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void Scatter_TensorOperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        ...
        auto args = static_cast<const Scatter_TensorOpFuncArgs *>(opArgs);
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto res = Scatter_(tileTensor0, tileTensor1, tileTensor2, args->axis_, args->reduce_);
...
static void Scatter_TensorOperationExeFunc3Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
...
static void Scatter_TensorOperationExeFunc4Dims(const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        SymbolicScalar self_firstDim = inputs[0].GetShape()[0];
        SymbolicScalar self_secondDim = inputs[0].GetShape()[1];
        SymbolicScalar self_thirdDim = inputs[0].GetShape()[2];
                ...
                LOOP("LOOP_L2_mIdx", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(0, mloop, 1)) {
                    LOOP("LOOP_L3_nIdx", FunctionType::DYNAMIC_LOOP, nIdx, LoopRange(0, nloop, 1)) {
                        ...
                        auto tileTensor1 = View(inputs[1],
                            ...
                            {bIdx * firstViewShape, sIdx * secondViewShape, mIdx * thirdViewShape,
                                nIdx * fourthViewShape});
                        auto tileTensor2 = View(inputs[2],
                            {firstViewShape, secondViewShape, thirdViewShape, fourthViewShape},
                            {std::min(src_firstDim - bIdx * firstViewShape, firstViewShape),
                                std::min(src_secondDim - sIdx * secondViewShape, secondViewShape),
...
class Scatter_TensorOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<Scatter_TensorOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestScatter_Tensor, Scatter_TensorOperationTest,
    ::testing::ValuesIn(GetOpMetaData<Scatter_TensorOpMetaData>(
        {Scatter_TensorOperationExeFunc2Dims, Scatter_TensorOperationExeFunc3Dims, Scatter_TensorOperationExeFunc4Dims},
        "Scatter_Tensor")));

TEST_P(Scatter_TensorOperationTest, TestScatter_Tensor) {
    ...
    auto args = Scatter_TensorOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data), axis, reduce);
    auto testCase = CreateTestCaseDesc<Scatter_TensorOpMetaData>(GetParam(), &args);
