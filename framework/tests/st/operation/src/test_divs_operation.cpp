...
struct DivsOpFuncArgs : public OpFuncArgs {
    DivsOpFuncArgs(const Element& value, const std::vector<int64_t>& viewShape, const std::vector<int64_t> tileShape)
...
static void DivsOperationExeFuncDoubleCut(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
    ...
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1))
        {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1))
            {
                ...
                auto res = Div(tileTensor0, args->value_);
...
static void DivsOperationExeFuncTripleCut(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                          const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
static void DivsOperationExeFuncQuadrupleCut(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                             const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
INSTANTIATE_TEST_SUITE_P(TestDivs, DivsOperationTest,
                         ::testing::ValuesIn(GetOpMetaData<DivsOpMetaData>({DivsOperationExeFuncDoubleCut,
                                                                            DivsOperationExeFuncTripleCut,
                                                                            DivsOperationExeFuncQuadrupleCut},
                                                                           "Divs")));
...
TEST_P(DivsOperationTest, TestDivs)
{
    ...
    auto testCase = CreateTestCaseDesc<DivsOpMetaData>(GetParam(), &args);
