...
namespace {
struct IndexAdd_OpFuncArgs : public OpFuncArgs {
    IndexAdd_OpFuncArgs(
...
struct IndexAddOp_MetaData {
    explicit IndexAddOp_MetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void IndexAdd_OperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        ...
        auto args = static_cast<const IndexAdd_OpFuncArgs *>(opArgs);
        int axis = args->axis_;
...
static void IndexAdd_OperationExeFunc3Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        SymbolicScalar self_firstDim = inputs[0].GetShape()[0];
        SymbolicScalar self_secondDim = inputs[0].GetShape()[1];
...
static void IndexAdd_OperationExeFunc4Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        SymbolicScalar self_firstDim = inputs[0].GetShape()[0];
        SymbolicScalar self_secondDim = inputs[0].GetShape()[1];
...
static void IndexAdd_OperationExeFunc5Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        SymbolicScalar self_firstDim = inputs[0].GetShape()[0];
        SymbolicScalar self_secondDim = inputs[0].GetShape()[1];
...
class IndexAdd_OperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<IndexAddOp_MetaData> {};

INSTANTIATE_TEST_SUITE_P(TestIndexAdd_, IndexAdd_OperationTest,
    ::testing::ValuesIn(GetOpMetaData<IndexAddOp_MetaData>({IndexAdd_OperationExeFunc2Dims, IndexAdd_OperationExeFunc3Dims,
    IndexAdd_OperationExeFunc4Dims, IndexAdd_OperationExeFunc5Dims}, "IndexAdd_")));
...
TEST_P(IndexAdd_OperationTest, TestIndexAdd_) {
    ...
    std::vector<OpFunc> opFuncs = {IndexAdd_OperationExeFunc2Dims, IndexAdd_OperationExeFunc3Dims,
        IndexAdd_OperationExeFunc4Dims, IndexAdd_OperationExeFunc5Dims};
