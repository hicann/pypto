...
static void CeilOperationExeFunc2Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
...
static void CeilOperationExeFunc3Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                      const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
static void CeilOperationExeFunc4Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                      const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
INSTANTIATE_TEST_SUITE_P(TestCeil, CeilOperationTest,
                         ::testing::ValuesIn(GetOpMetaData<CeilOpMetaData>(
                             {CeilOperationExeFunc2Dims, CeilOperationExeFunc3Dims, CeilOperationExeFunc4Dims},
                             "Ceil")));
