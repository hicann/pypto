...
/*!
 * \file test_Scatter__operation.cpp
 * \brief
 */
...
struct Scatter_OpMetaData {
    explicit Scatter_OpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void Scatter_OperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        auto args = static_cast<const Scatter_OpFuncArgs *>(opArgs);
        const int64_t firstViewShape = args->viewShape_[0];
        const int64_t secondViewShape = args->viewShape_[1];

        /* 根据当前Scatter_实现，做tile切分时，axis轴取对应输入的axis轴的shape大小，即axis轴不做切分
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto res = Scatter_(tileTensor0, tileTensor1, args->value_, args->axis_, args->reduce_);
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                TileShape::Current().SetVecTile(args->tileShape_);
                auto res = Scatter_(tileTensor0, tileTensor1, args->value_, args->axis_);
                Assemble(res, {bIdx * firstViewShape, sIdx * secondViewShape}, outputs[0]);
            }
        }
    }
            ...
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                LOOP("LOOP_L2_mIdx", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(0, mloop, 1)) {
                    LOOP("LOOP_L3_nIdx", FunctionType::DYNAMIC_LOOP, nIdx, LoopRange(0, nloop, 1)) {
                        ...
                        auto tileTensor1 = View(inputs[1], {firstViewShape, secondViewShape, thirdViewShape, fourthViewShape},
                            {std::min(idx_firstDim - bIdx * firstViewShape, firstViewShape),
                                std::min(idx_secondDim - sIdx * secondViewShape, secondViewShape),
...
class Scatter_OperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<Scatter_OpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestScatter_, Scatter_OperationTest,
    ::testing::ValuesIn(GetOpMetaData<Scatter_OpMetaData>(
        {Scatter_OperationExeFunc2Dims, Scatter_OperationExeFunc3Dims, Scatter_OperationExeFunc4Dims, 
        Scatter_OperationExeFunc2DimsNoReduceOp},
        "Scatter_")));
