...
struct FloorDivsOpMetaData {
    explicit FloorDivsOpMetaData(const OpFunc& opFunc, const nlohmann::json& test_data)
...
TEST_P(FloorDivsOperationTest, TestFloorDivs)
{
    ...
    auto args = FloorDivsOpFuncArgs(value, GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<FloorDivsOpMetaData>(GetParam(), &args);
