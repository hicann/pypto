...
/*!
 * \file test_logical_not_operation.cpp
 * \brief
 */
...
namespace {
struct LogicalNotOpFuncArgs : public OpFuncArgs {
    LogicalNotOpFuncArgs(const std::vector<int64_t>& viewShape, const std::vector<int64_t> tileShape)
...
struct LogicalNotOpMetaData {
    explicit LogicalNotOpMetaData(const OpFunc& opFunc, const nlohmann::json& test_data)
...
static void LogicalNotOperationExeFunc1Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
...
static void LogicalNotOperationExeFunc2Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                            const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
static void LogicalNotOperationExeFunc3Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                            const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
static void LogicalNotOperationExeFunc4Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                            const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
            ...
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1))
            {
                LOOP("LOOP_L2_mIdx", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(0, mloop, 1))
                {
                    LOOP("LOOP_L3_nIdx", FunctionType::DYNAMIC_LOOP, nIdx, LoopRange(0, nloop, 1))
                    {
                        ...
                        TileShape::Current().SetVecTile(args->tileShape_);
                        auto res = LogicalNot(tileTensor0);
                        Assemble(res,
                                 {bIdx * firstViewShape, sIdx * secondViewShape, mIdx * thirdViewShape,
                                  nIdx * fourthViewShape},
                                 outputs[0]);
...
class LogicalNotOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<LogicalNotOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestLogicalNot, LogicalNotOperationTest,
                         ::testing::ValuesIn(GetOpMetaData<LogicalNotOpMetaData, 1>(
                             {LogicalNotOperationExeFunc1Dims, LogicalNotOperationExeFunc2Dims,
                              LogicalNotOperationExeFunc3Dims, LogicalNotOperationExeFunc4Dims},
                             "LogicalNot")));
...
TEST_P(LogicalNotOperationTest, TestLogicalNot)
{
    ...
    auto testCase = CreateTestCaseDesc<LogicalNotOpMetaData>(GetParam(), &args);
