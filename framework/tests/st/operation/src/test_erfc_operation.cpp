...
/*!
 * \file test_erfc_operation.cpp
 * \brief Test for erfc operation
 */
...
namespace {
struct ErfcOpFuncArgs : public OpFuncArgs {
    ErfcOpFuncArgs(const std::vector<int64_t>& viewShape, const std::vector<int64_t> tileShape)
...
struct ErfcOpMetaData {
    explicit ErfcOpMetaData(const OpFunc& opFunc, const nlohmann::json& test_data)
    ...
    const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs, const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
        ...
        const struct ErfcOpFuncArgs* args = static_cast<const ErfcOpFuncArgs*>(opArgs);
    ...
    const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs, const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
        ...
        auto args = static_cast<const ErfcOpFuncArgs*>(opArgs);
        const int firstViewShape = args->viewShape_[0];
        const int secondViewShape = args->viewShape_[1];
        const int thirdViewShape = args->viewShape_[2];
        const int fourthViewShape = args->viewShape_[3];
...
TEST_P(ErfcOperationTest, TestErfc)
{
    ...
    auto args = ErfcOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<ErfcOpMetaData>(GetParam(), &args);
