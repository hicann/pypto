    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            Tensor tileTensor0 = View(inputs[0], {firstViewShape},
                {std::min(firstTensorDim - bIdx * firstViewShape, firstViewShape)}, {bIdx * firstViewShape});
            Tensor tileTensor1;
            IF(secondTensorDim == 1) {
                tileTensor1 = View(inputs[1], {1}, {1}, {0});
            }
            ELSE {
                tileTensor1 = View(inputs[1], {firstViewShape},
                    {std::min(secondTensorDim - bIdx * firstViewShape, firstViewShape)}, {bIdx * firstViewShape});
            }
