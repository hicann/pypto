...
static void SqrtOperationExeFunc3Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                      const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
static void SqrtOperationExeFunc4Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                      const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
...
INSTANTIATE_TEST_SUITE_P(TestSqrt, SqrtOperationTest,
                         ::testing::ValuesIn(GetOpMetaData<SqrtOpMetaData>(
                             {SqrtOperationExeFunc2Dims, SqrtOperationExeFunc3Dims, SqrtOperationExeFunc4Dims},
                             "Sqrt")));
