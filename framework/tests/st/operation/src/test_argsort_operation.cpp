...
static void ArgSortOperationExeFunc2Dims(const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        if (inputs[0].GetShape().size() == 1) {
            ...
            LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(loop[IDX_DIM0])) {
                ...
                auto viewTensor = View(inputs[0], args->viewShape_, {std::min(firstDim - bIdx * firstViewShape, firstViewShape)}, offset);
...
static void ArgSortOperationExeFunc3Dims(const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        const struct ArgSortOpFuncArgs *args = static_cast<const ArgSortOpFuncArgs *>(opArgs);
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
...
static void ArgSortOperationExeFunc4Dims(const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        const struct ArgSortOpFuncArgs *args = static_cast<const ArgSortOpFuncArgs *>(opArgs);
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
...
INSTANTIATE_TEST_SUITE_P(TestArgSort, ArgSortOperationTest, ::testing::ValuesIn(
    GetOpMetaData<ArgSortOpMetaData>({ArgSortOperationExeFunc2Dims, ArgSortOperationExeFunc3Dims, ArgSortOperationExeFunc4Dims}, "ArgSort")
));
