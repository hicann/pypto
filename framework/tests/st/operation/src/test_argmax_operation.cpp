...
void ArgMaxOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
...
void ArgMax3DOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                 const OpFuncArgs* opArgs) {
    auto args = static_cast<const ArgMaxOpFuncArgs *>(opArgs);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
void ArgMax4DOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                 const OpFuncArgs* opArgs) {
    auto args = static_cast<const ArgMaxOpFuncArgs *>(opArgs);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
INSTANTIATE_TEST_SUITE_P(TestArgMax, ArgMaxOperationTest, ::testing::ValuesIn(
    GetOpMetaData<ArgMaxOpMetadata>({ArgMaxOperationExeFunc, ArgMax3DOperationExeFunc,
        ArgMax4DOperationExeFunc}, "ArgMax")));

TEST_P(ArgMaxOperationTest, TestArgMax) {
    auto test_data = GetParam().test_data_;
    auto args = ArgMaxOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data),
