...
struct AddsOpFuncArgs : public OpFuncArgs {
    AddsOpFuncArgs(const Element &value, const std::vector<int64_t> &viewShape, const std::vector<int64_t> tileShape)
...
struct AddsOpMetaData {
    explicit AddsOpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void AddsOperationExeFuncDoubleCut(
...
static void AddsOperationExeFuncTripleCut(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
...
static void AddsOperationExeFuncQuadrupleCut(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
...
class AddsOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<AddsOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestAdds, AddsOperationTest,
    ::testing::ValuesIn(GetOpMetaData<AddsOpMetaData>(
        {AddsOperationExeFuncDoubleCut, AddsOperationExeFuncTripleCut, AddsOperationExeFuncQuadrupleCut}, "Adds")));

TEST_P(AddsOperationTest, TestAdds) {
    ...
    auto args = AddsOpFuncArgs(value, GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<AddsOpMetaData>(GetParam(), &args);
