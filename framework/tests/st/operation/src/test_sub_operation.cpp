...
struct SubOpFuncArgs : public OpFuncArgs {
    SubOpFuncArgs(const std::vector<int64_t>& viewShape, const std::vector<int64_t> tileShape)
...
struct SubOpMetaData {
    explicit SubOpMetaData(const OpFunc& opFunc, const nlohmann::json& test_data)
...
static void SubOperationExeFunc2Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
...
static void SubOperationExeFunc3Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                     const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]})
    {
...
static void SubOperationExeFunc4Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                     const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]})
    {
...
static void SubOperationExeFunc5Dims(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                     const OpFuncArgs* opArgs)
{
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]})
    {
...
INSTANTIATE_TEST_SUITE_P(
    TestSub, SubOperationTest,
    ::testing::ValuesIn(GetOpMetaData<SubOpMetaData>({SubOperationExeFunc2Dims, SubOperationExeFunc3Dims,
                                                      SubOperationExeFunc4Dims, SubOperationExeFunc5Dims},
                                                     "Sub")));

TEST_P(SubOperationTest, TestSub)
{
    auto test_data = GetParam().test_data_;
    auto args = SubOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<SubOpMetaData>(GetParam(), &args);
